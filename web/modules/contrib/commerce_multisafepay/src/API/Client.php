...
class Client {
    ...
    public function processAPIRequest($http_method, $api_method, $http_body = NULL) {
        if (empty($this->api_key)) {
            $this->exceptionHelper->PaymentGatewayException("Please configure your MultiSafepay API Key.");
        }
        ...
        if (curl_errno($ch)) {
            $this->exceptionHelper->PaymentGatewayException("Unable to communicate with the MultiSafepay payment server (" . curl_errno($ch) . "): " . curl_error($ch) . ".");
        }
