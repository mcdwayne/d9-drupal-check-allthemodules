...
class NgLightboxTest extends KernelTestBase {
  ...
  public function testPatternMatching() {
    ...
    $config->set('patterns', $node->toUrl())->save();
    ...
    $node = Node::create(['type' => 'page', 'title' => $this->randomString()]);
    $node->save();
    $this->assertLightboxNotEnabled(Link::fromTextAndUrl('Normal Path', $node->toUrl()));
  ...
  protected function assertContains($needle, $haystack, $message = '') {
    if (empty($message)) {
      $message = t('%needle was found within %haystack', ['%needle' => $needle, '%haystack' => $haystack]);
    }
  ...
  protected function assertNotContains($needle, $haystack, $message = '') {
    if (empty($message)) {
      $message = t('%needle was not found within %haystack', ['%needle' => $needle, '%haystack' => $haystack]);
    }
    return $this->assertTrue(stripos($haystack, $needle) === FALSE, $message);
  }
