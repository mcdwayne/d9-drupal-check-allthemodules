...
class BaiduAnalyticsAdminSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    for ($i = 1; $i < 6; $i++) {
      ...
      if (\Drupal::moduleHandler()->moduleExists('token')) {
        $form['baidu_analytics_custom_var']['baidu_analytics_custom_var_table'][$i]['name']['#element_validate'][] = 'token_element_validate';
        $form['baidu_analytics_custom_var']['baidu_analytics_custom_var_table'][$i]['value']['#element_validate'][] = 'token_element_validate';
      }
  ...
  public function validateForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    foreach ($form_state->getValue(['baidu_analytics_custom_var', 'baidu_analytics_custom_var_table']) as $custom_var) {
      $form_state->setValue(['baidu_analytics_custom_var', 'baidu_analytics_custom_var_table', $custom_var['slot'], 'name'], trim($custom_var['name']));
      $form_state->setValue(['baidu_analytics_custom_var', 'baidu_analytics_custom_var_table', $custom_var['slot'], 'value'], trim($custom_var['value']));

      // Validate empty names/values.
      if (empty($custom_var['name']) && !empty($custom_var['value'])) {
        $form_state->setErrorByName("baidu_analytics_custom_var][baidu_analytics_custom_var_table][" . $custom_var['slot'] . "][name", t('The custom variable @slot-number requires a <em>Name</em> if a <em>Value</em> has been provided.', [
          '@slot-number' => $custom_var['slot']
          ]));
      }
      elseif (!empty($custom_var['name']) && empty($custom_var['value'])) {
        $form_state->setErrorByName("baidu_analytics_custom_var][baidu_analytics_custom_var_table][" . $custom_var['slot'] . "][value", t('The custom variable @slot-number requires a <em>Value</em> if a <em>Name</em> has been provided.', [
           '@slot-number' => $custom_var['slot']
          ]));
      }
    }
  ...
  function baidu_analytics_token_element_validate(&$element, &$form_state) {
    ...
    $tokens = \Drupal::service('token')->scan($value);
    $invalid_tokens = $this->_baidu_analytics_get_forbidden_tokens($tokens);
    if ($invalid_tokens) {
      $form_state->setError($element, $this->t('The %element-title is using the following forbidden tokens with personal identifying information: @invalid-tokens.', array(
   ...
   */
  public function _baidu_analytics_get_forbidden_tokens($value) {
    $invalid_tokens = array();
    $value_tokens = is_string($value) ? \Drupal::service('token')->scan($value) : $value;

    foreach ($value_tokens as $tokens) {
      if (array_filter($tokens, '\Drupal\baidu_analytics\Form\BaiduAnalyticsAdminSettingsForm::_baidu_analytics_contains_forbidden_token')) {
   ...
   */
  public function _baidu_analytics_contains_forbidden_token($token_string) {
