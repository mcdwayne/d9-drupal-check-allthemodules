...
class JssorFieldFormatter extends ImageFormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    if (!empty($image_style_setting)) {
      ...
      $images_cache_tags = $image_style->getCacheTags();
    }
    ...
    foreach ($files as $delta => $file) {
      ...
      if (!empty($image_style_setting)) {
        $image_style = $this->imageStyleStorage->load($image_style_setting);
        $image_cache_tags = $image_style->getCacheTags();
      }

      $images_cache_tags = Cache::mergeTags($images_cache_tags, $file->getCacheTags());
      $image_cache_tags = Cache::mergeTags($image_cache_tags, $file->getCacheTags());
