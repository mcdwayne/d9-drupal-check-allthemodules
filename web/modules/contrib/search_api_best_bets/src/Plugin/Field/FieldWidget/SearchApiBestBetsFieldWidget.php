...
class SearchApiBestBetsFieldWidget extends WidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $element['elevate'] = array(
      '#type' => 'textfield',
      '#title' => $this->getSetting('elevate_label'),
      '#placeholder' => $this->getSetting('elevate_placeholder'),
      '#description' => t('Write the elevate queries in the field - separate multiple by comma.'),
      '#default_value' => isset($default_values['elevate']) ? $default_values['elevate'] : NULL,
      '#maxlength' => 64,
      '#required' => $element['#required'],
    );

    if (!$this->getSetting('disable_exclude')) {
      $element['exclude'] = array(
        '#type' => 'textfield',
        '#title' => $this->getSetting('exclude_label'),
        '#placeholder' => $this->getSetting('exclude_placeholder'),
        '#description' => $this->getSetting('exclude_description'),
        '#default_value' => isset($default_values['exclude']) ? $default_values['exclude'] : NULL,
        '#maxlength' => 64,
        '#required' => $element['#required'],
      );
    }
  ...
  /**
   * Massage a comma separated string to the array style expected by the field
   * type.
   *
   * @param string $values
  ...
  private function massageSeparatedValues($values, $exclude = FALSE) {
    ...
    foreach ($source_values as $value) {
      if (trim($value)) {
        $fixed_values[] = [
          'query_text' => Unicode::strtolower(trim($value)),
          'exclude' => (int) $exclude
        ];
      }
    }
   ...
   */
  private function massageDefaultArrayValues($items) {
    $fixed_values = [
      'elevate' => [],
      'exclude' => []
    ];
