...
class BestBetsProcessor extends ProcessorPluginBase implements PluginFormInterface {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($this->index->getDatasources() as $datasource_id => $datasource) {
      ...
      $form['fields'][$entity_type] = array(
        ...
        '#default_value' => isset($config_fields[$entity_type]) ? $config_fields[$entity_type] : array(),
        '#options' => $this->getFieldOptions($entity_type, $datasource),
        '#multiple' => TRUE,
      );
    }

    $form['query_handler'] = array(
      '#type' => 'select',
    ...
    $form['result_elevated_flag'] = array(
      '#type' => 'select',
      ...
      '#options' => [
        'query_handler' => t('Query handler plugin'),
        'local' => t('Process locally'),
      ],
    );
  ...
  public function validateConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    if (isset($values['fields']) && is_array($values['fields'])) {
      foreach ($values['fields'] as $entity => $fields) {
    ...
    if ($error) {
      $form_state->setErrorByName('', t('Choose at least one field to enable the best bets feature.'));
    }
  }
  ...
  private function getFieldOptions($entity_type, $datasource) {
    $field_map = $this->entityManager->getFieldMapByFieldType('search_api_best_bets');
  ...
  /**
   * Find and return a list of best bets handler plugins that support the
   * current backend.
   *
   * @return array
  ...
  public function preprocessSearchQuery(QueryInterface $query) {
    ...
    if ($plugin_id = $config['query_handler']) {
      ...
      $instance = $this->queryHandlerManager->createInstance($plugin_id, array());
  ...
  public function postprocessSearchResults(ResultSetInterface $results) {
    ...
    if ($plugin_id = $config['query_handler']) {
      ...
      // Set the elevated locally based on elevated items send to search backend.
      else {
