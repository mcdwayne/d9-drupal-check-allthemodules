...
/**
 * Plugin implementation of the 'tracking_number_link' formatter.
 *
 * @FieldFormatter(
 *   id = "tracking_number_link",
 ...
 * )
 */
class TrackingNumberLinkFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function settingsSummary() {
    ...
    $summary[] = $this->getSetting('new_window') ? t('Link opens in new window') : t('Link opens in same window');
    ...
    foreach ($items as $delta => $item) {
      ...
      if (!$item->isEmpty() && $this->trackingNumberTypeManager->getDefinition($item->type, FALSE)) {
        ...
        $elements[$delta]['item']['number'] = [
          '#type' => 'link',
          '#url' => $type_plugin->getTrackingUrl($item->value),
          '#title' => $item->value,
          '#attributes' => [
            'class' => [
              'number',
        ...
        // Respect open in new window setting.
        if ($this->getSetting('new_window')) {
          $elements[$delta]['item']['number']['#attributes']['target'] = '_blank';
        }
