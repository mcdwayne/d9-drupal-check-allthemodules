...
abstract class UserProtectWebTestBase extends WebTestBase {
  ...
  protected function createProtectionRule($entity_id, array $protections = [], $entity_type = 'user_role', array $values = []) {
    ...
    $this->assertTrue($protection_rule instanceof ProtectionRuleInterface, SafeMarkup::format('Created protection rule %rule.', ['%rule' => $protection_rule->id()]));
   ...
   *   NULL otherwise.
   */
  protected function userprotectPostForm($path, array $edit, $submit, array $options = [], array $headers = [], $form_html_id = NULL) {
    if (isset($path)) {
    ...
    if ($this->parse()) {
      ...
      $this->fail(SafeMarkup::format('Found the requested form fields at @path', ['@path' => $path]));
    }
  }
