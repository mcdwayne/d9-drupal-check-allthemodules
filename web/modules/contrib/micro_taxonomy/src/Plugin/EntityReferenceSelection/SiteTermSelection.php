 ...
 *   id = "site_default:taxonomy_term",
 *   label = @Translation("Taxonomy Term selection (with support for micro sites)"),
 *   entity_types = {"taxonomy_term"},
 *   group = "site_default",
 *   base_plugin_label = @Translation("Taxonomy Term selection (with support for micro sites)"),
 *   weight = 1
 * )
 */
class SiteTermSelection extends DefaultSelection {
  ...
  protected function buildEntityQuery($match = NULL, $match_operator = 'CONTAINS') {
    ...
    if ($active_site = $this->negotiator()->getActiveSite()) {
      ...
      $condition_or->condition('site_id', $active_site->id());
      $condition_or->condition('site_all', 1);
