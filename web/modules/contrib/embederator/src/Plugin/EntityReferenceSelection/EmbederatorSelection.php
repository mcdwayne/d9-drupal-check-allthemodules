...
namespace Drupal\embederator\Plugin\EntityReferenceSelection;
...
/**
 * Embederator plugin implementation of the Entity Reference Selection plugin.
 *
 * @EntityReferenceSelection(
 *   id = "default:embederator",
 *   label = @Translation("Embederator"),
 *   group = "default",
 *   entity_types = {"embederator"},
 *   weight = 0
 * )
 */
class EmbederatorSelection extends DefaultSelection {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['negate'] = [
      ...
      '#title' => $this->t('Which Embederator types should be allowed?'),
    ...
    if ($bundle_options) {
      $form['target_bundles_drag_drop'] = [
        ...
        '#prefix' => '<h5>' . $this->t('Embederator types') . '</h5>',
        '#suffix' => '<div class="description">' . $this->t('Selection of Embederator types for this field. Select none to allow all Embederator types.') . '</div>',
      ];
    ...
    if (!count($bundle_options)) {
      $form['allowed_bundles_explain'] = [
        '#type' => 'markup',
        '#markup' => $this->t('You did not add any Embederator types yet, click <a href=":here">here</a> to add one.', [':here' => Url::fromRoute('embederator_type.add')->toString()]),
      ];
    }
   ...
   *
   * @return array
   *   A list of arrays keyed by the embederator type machine name
   *   with the following properties.
   *     - label: The label of the embederator type.
   *     - weight: The weight of the embederator type.
   */
  public function getSortedAllowedTypes() {
    $return_bundles = [];

    $bundles = $this->entityTypeBundleInfo->getBundleInfo('embederator');
