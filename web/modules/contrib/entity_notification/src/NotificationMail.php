...
class NotificationMail {
  ...
  protected function notificationMail($key, &$message, $params) {
    ...
    if ($params['config']['from']) {
      $message['from'] = $this->token->replace($params['config']['from'], [$entity->getEntityTypeId() => $entity]);
      $message['headers']['From'] = $this->token->replace($params['config']['from'], [$entity->getEntityTypeId() => $entity]);
    }
