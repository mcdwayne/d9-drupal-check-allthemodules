...
class CanvasUsers extends Canvas{
   ...
   * Example:
   *
   *    $canvas_api = \Drupal::service('canvas_api.users');
   ...
   *    $users = $canvas_api->users();
   *
   * @param string $rootAccountID
   * @return array
   *   An array of Users
   */
  public function users($rootAccountID = 1){
    $this->path = "accounts/$rootAccountID/users";
    return $this->get();
  }

  /**
   * Get a Canvas user profile
   *
   * See @link https://canvas.instructure.com/doc/api/users.html#method.profile.settings @endlink
   *
   * Example:
   *    $canvas_api = \Drupal::service('canvas_api.users');
   *    $sisUserID = 'sis_user_id:DOE123';
   *    $user = $canvas_api->getUserProfile($sisUserID);
   *
   * @param $userID
   * @return array
   */
  public function getUserProfile($userID){
    $this->path = "users/$userID/profile";
    return $this->get();
  }
