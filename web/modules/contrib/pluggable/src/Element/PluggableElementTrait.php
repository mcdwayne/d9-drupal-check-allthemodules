...
namespace Drupal\pluggable\Element;
...
/**
 * Provides a trait for Pluggable form elements.
 *
 * Allows form elements to use #pluggable_element_submit, a substitute
 ...
 */
trait PluggableElementTrait {

  /**
   * Attaches the #pluggable_element_submit functionality.
   *
   * @param array $element
  ...
  public static function attachElementSubmit(array $element, FormStateInterface $form_state, array &$complete_form) {
    if (isset($complete_form['#pluggable_element_submit_attached'])) {
      return $element;
    }
    ...
    $complete_form['#pluggable_element_submit_attached'] = TRUE;

    return $element;
  }

  /**
   * Confirms that #pluggable_element_submit handlers can be run.
   *
   * @param array $element
   *   The form element.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  /**
   * Submits elements by calling their #pluggable_element_submit callbacks.
   *
   * Form API has no #element_submit, requiring us to simulate it by running
   * the #pluggable_element_submit handlers either in the last step of
  ...
  /**
   * Calls the #pluggable_element_submit callbacks recursively.
   *
   * @param array &$element
   *   The current element.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  public static function doExecuteSubmitHandlers(array &$element, FormStateInterface $form_state) {
    ...
    if (!empty($element['#pluggable_element_submit'])) {
      foreach ($element['#pluggable_element_submit'] as $callback) {
