 ...
 *   '#plugin_type' => 'alpha_process_input',
 *   '#plugin_id' => 'alpha_process_dom',
 *   '#default_value' => [
 *     'html' => '',
 *   ],
 * ];
 * @endcode
 *
 * @FormElement("pluggable_plugin_configuration")
 */
class PluggablePluginConfiguration extends FormElement {

  use PluggableElementTrait;
  ...
  public static function processPluginConfiguration(array &$element, FormStateInterface $form_state, array &$complete_form) {
    if (empty($element['#plugin_type'])) {
      throw new \InvalidArgumentException('The pluggable_plugin_configuration #plugin_type property is required.');
    }
    if (!is_array($element['#default_value'])) {
      throw new \InvalidArgumentException('The pluggable_plugin_configuration #default_value must be an array.');
    }
    if (empty($element['#plugin_id'])) {
      // A plugin hasn't been selected yet.
      return $element;
    }
    if ($element['#plugin_id'] == '_none') {
      // A plugin hasn't been selected yet.
      return $element;
    }
  ...
  public static function validatePluginConfiguration(array &$element, FormStateInterface $form_state, array &$complete_form) {
    if (!empty($element['#plugin_id']) && $element['#plugin_id'] != '_none') {
