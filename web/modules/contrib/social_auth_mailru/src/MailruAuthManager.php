...
namespace Drupal\social_auth_mailru;
...
/**
 * Contains all the logic for Mailru login integration.
 */
class MailruAuthManager extends OAuth2Manager {
  ...
  /**
   * The Mailru client object.
   *
   * @var \League\OAuth2\Client\Provider\Mailru
   */
  protected $client;
  /**
   * The Mailru access token.
   *
   ...
   * The Mailru user.
   *
   * @var \League\OAuth2\Client\Provider\MailruUser
   */
  protected $user;
  ...
  public function __construct(LoggerChannelFactoryInterface $logger_factory, EventDispatcherInterface $event_dispatcher, EntityFieldManagerInterface $entity_field_manager, UrlGeneratorInterface $url_generator, ConfigFactory $configFactory) {
    ...
    $this->config             = $configFactory->getEditable('social_auth_mailru.settings');
  }
   ...
   *
   * @return League\OAuth2\Client\Provider\MailruUser
   *   User info returned by the Mailru.
   */
  ...
  /**
   * Returns the Mailru login URL where user will be redirected.
   *
   * @return string
   *   Absolute Mailru login URL where user will be redirected
   */
  public function getMailruLoginUrl() {
    $scopes = [];
