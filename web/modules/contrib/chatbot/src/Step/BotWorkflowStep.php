...
class BotWorkflowStep implements BotWorkflowStepInterface {
  ...
  public function __construct($stepName, $machineName, $questionMessage) {
    ...
    elseif (is_callable($questionMessage)) {
      ...
      throw new \Exception('Argument 3 for \Drupal\chatbot\Step\BotWorkflowStep cannot be empty and must either be callable, and array, or implement \Drupal\chatbot\Message\MessageInterface.');
    }
  ...
  public function setInvalidResponseMessage($invalidMessage) {
    if ($invalidMessage instanceof MessageInterface) {
      $this->invalidResponseMessage = [$invalidMessage];
    }
  ...
  public function processResponse(array $receivedMessage) {
    ...
    if (!$this->validateResponse($receivedMessage)) {
      ...
      $handler = $this->getHandlerForMessage($receivedMessage['message_content']);
  ...
  private function getHandlerForMessage($message) {
    if (in_array($message, array_keys($this->responseHandlers))) {
      return $this->responseHandlers[$message];
    }
    foreach ($this->responseHandlers as $key => $handler) {
      $conditionMatchType = isset($handler['conditionMatchType']) ? $handler['conditionMatchType'] : NULL;
      $conditionMatchText = isset($handler['conditionMatchText']) ? $handler['conditionMatchText'] : NULL;
      switch ($conditionMatchType) {
        case "contains":
          if (strstr($message, $conditionMatchText)) {
            return $handler;
          }
          break;

        case "matches":
          if ($message == $conditionMatchText) {
            return $handler;
          }
          break;
      }
    ...
    return isset($this->responseHandlers['*']) ? $this->responseHandlers['*'] : NULL;
  }
