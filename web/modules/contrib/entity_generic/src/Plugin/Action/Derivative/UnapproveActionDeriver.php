 ...
 */
class UnapproveActionDeriver extends EntityActionDeriverBase {
  ...
  public function getDerivativeDefinitions($base_plugin_definition) {
    if (empty($this->derivatives)) {
      ...
      foreach ($this->getApplicableEntityTypes() as $entity_type_id => $entity_type) {
        ...
        $definition['label'] = $this->t('Unapprove @entity_type', ['@entity_type' => $entity_type->getSingularLabel()]);
  ...
  protected function isApplicable(EntityTypeInterface $entity_type) {
    return $entity_type->hasKey('approved');
  }
