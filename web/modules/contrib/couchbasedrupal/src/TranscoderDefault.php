...
/**
 * Use this to implement any
 * couchbase bucket transcoder
 */
class TranscoderDefault implements TranscoderInterface {
   ...
   * @internal
   */
  protected $COUCHBASE_DEFAULT_ENCOPTS = array(
   ...
   * @internal
   */
  protected $COUCHBASE_OLD_ENCOPTS = array(
   ...
   * @internal
   */
  protected $COUCHBASE_DEFAULT_DECOPTS = array(
      'jsonassoc' => false
  );
  ...
  public function encode($value) {
    ...
    $flags = 0;
    $datatype = 0;
    $sertype = $this->options['sertype'];
    $cmprtype = $this->options['cmprtype'];
    $cmprthresh = $this->options['cmprthresh'];
    $cmprfactor = $this->options['cmprfactor'];
    $vtype = gettype($value);
    if ($vtype == 'string') {
   ...
   */
  public function decode($bytes, $flags, $datatype) {
