...
apply_sql_function($X["fun"],$C)."</a>";echo"<span class='column hidden'>","<a href='".h($Rc.$zb)."' title='".lang(86)."' class='text'> â†“</a>";if(!$X["fun"]){echo'<a href="#fieldset-search" title="'.lang(41).'" class="text jsonly"> =</a>',script("qsl('a').onclick = partial(selectSearch, '".js_escape($z)."');");}echo"</span>";}$Cc[$z]=$X["fun"];next($M);}}$Ad=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
