...
/**
 * @file
 * Contains \Drupal\ert\Routing\RouteEnhancer.
 */
...
/**
 * Enhances Entity read time routes by adding proper information about the bundle name.
 */
class RouteEnhancer implements RouteEnhancerInterface {
  ...
  public function enhance(array $defaults, Request $request) {
    if (($bundle = $this->entityManager->getDefinition($defaults['entity_type_id'])->getBundleEntityType()) && isset($defaults[$bundle])) {
      // Entity Read time forms only need the actual name of the bundle they're dealing
  ...
  public function applies(Route $route) {
    return ($route->hasOption('read_time'));
  }
