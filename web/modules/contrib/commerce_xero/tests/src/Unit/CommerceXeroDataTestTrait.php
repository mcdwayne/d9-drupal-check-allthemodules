 ...
 *
 * @internal
 * @deprecated Use XeroDataTestTrait instead once that version is fixed.
 */
trait CommerceXeroDataTestTrait {
  ...
  protected function createTypedDataProphet() {
    ...
    $this->discovery = new AnnotatedClassDiscovery(
      [
        '\Drupal\xero\Plugin\DataType' => [__DIR__ . '/../../../../xero/src/Plugin/DataType/'],
  ...
  protected function mockTypedData($type, $value, $name = NULL, DataDefinitionInterface $itemDefinition = NULL) {
    ...
    if (class_exists($class)) {
      ...
      $this->fail(printf('class %s does not exist for type: %s', $class, $debug_type));
    }
