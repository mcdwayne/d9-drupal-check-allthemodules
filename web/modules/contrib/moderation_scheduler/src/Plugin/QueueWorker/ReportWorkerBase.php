...
abstract class ReportWorkerBase extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    $form = new static(
        $configuration, $plugin_id, $plugin_definition, $container->get('state'), $container->get('logger.factory')
    );
  ...
  protected function reportWork($worker, $item) {
    if ($this->state->get('moderation_scheduler_show_status_message')) {
    ...
    $this->logger->get('moderation_scheduler')->info('Queue @worker worker processed item with sequence @sequence created at @time', [
      '@worker' => $worker,
      '@sequence' => $item->sequence,
      '@time' => date_iso8601($item->created),
    ]);
