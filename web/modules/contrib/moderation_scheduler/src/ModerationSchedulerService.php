...
class ModerationSchedulerService {
   ...
   */
  public function __construct(ModuleHandler $moduleHandler, ConfigFactory $configFactory, EntityManager $entityManager, QueryFactory $entityQuery, Connection $database, LanguageManagerInterface $languageManager, EventDispatcherInterface $eventDispatcher) {
   ...
   * @throws \Drupal\Component\Plugin\Exception\InvalidPluginDefinitionException
   */
  public function fieldScheduledTimeRevision($nid, $langcode) {
    $node_storage = $this->entityManager->getStorage('node');
  ...
  public function cleanFieldScheduledTimeRevision($field) {
    ...
    $query2 = $db->delete('node__field_scheduled_time');
    $delete = $query1->execute();
    $delete1 = $query2->execute();
