...
namespace Drupal\gclient_storage\Controller;
...
/**
 * Defines a controller to serve public/gs Google Storage image styles.
 */
class GclientStorageImageStyleDownloadController extends ImageStyleDownloadController {

  /**
   * Generates a Google Storage derivative, given a style and image path.
   *
   * After generating an image, redirect it to the requesting agent. Only used
   * for public or gs schemes. Private scheme use the normal workflow:
  ...
  public function deliver(Request $request, $scheme, ImageStyleInterface $image_style) {
    ...
    if (!file_exists($derivative_uri)) {
      $lock_name = 'gclient_storage_image_style_deliver:' . $image_style->id() . ':' . Crypt::hashBase64($image_uri);
    ...
    if ($success) {
      // Perform a 302 Redirect to the new image derivative in Google Storage.
      ...
      $cacheableMetadata->setCacheMaxAge((int)$this->config('gclient_storage.settings')->get('stream_styles_ttl'));
