...
class ContentController extends ControllerBase {
  public function render() {
    ...
    foreach ($result as $row) {
      if ($row->published || $row->unpublished) {
        $rows[] = array(
          'logged' => $row->logged,
          'type' => $row->type,
    ...
    $page[] = \Drupal::formBuilder()->getForm('Drupal\sitelog\Form\ContentForm');
    $page[] = array(
      '#markup' => '</div></div>',
    );
    ...
    $page[] = array(
      '#theme' => 'image',
      '#uri' => drupal_get_path('module', 'sitelog') . '/img/blues.png',
      '#alt' => "",
      '#width' => 150,
      '#height' => 12,
