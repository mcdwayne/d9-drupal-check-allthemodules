...
(function (Drupal, d3, drupalSettings) {
  Drupal.behaviors.sitelogComments = {
    attach: function (context, settings) {
      ...
      // define color scheme
      const scheme = d3.scaleSequential(d3.interpolatePurples);
      ...
      // load data
      const data = JSON.parse(drupalSettings.sitelog.comments.data);
      ...
      function render(period = 90) {
        ...
        y.domain([
          d3.min(subset, d => {
            return d.posted - 1 < 0 ? 0 : d.posted - 1;
          }),
          d3.max(subset, d => {
            return d.posted + 1;
          }),
