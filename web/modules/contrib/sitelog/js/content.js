...
(function (Drupal, d3, drupalSettings) {
  Drupal.behaviors.sitelogContent = {
    attach: function (context, settings) {
      ...
      d3.selectAll("input")
        .on("change", function() {
          ...
          status = d3.select('input[name=status]:checked').attr('value');
          type = d3.select('input[name=type]:checked').attr('value');
      ...
      // define color scheme
      const scheme = d3.scaleSequential(d3.interpolateBlues);
      ...
      // load data
      const data = JSON.parse(drupalSettings.sitelog.content.data);
      ...
      // initialise chart
      render();
      function render(period = 90, status = 'published') {
        ...
        // filter data
        const typeSet = data.filter(d => {
          return d.type == type;
        });
        const subset = typeSet.filter((d, i) => {
          return i >= (typeSet.length - period);
        });
        ...
        y.domain([
          d3.min(subset, d => {
            return d[status] - 1 < 0 ? 0 : d[status] - 1;
          }),
          d3.max(subset, d => {
            return d[status] + 1;
          }),
