  ...
  protected function chargeCard(OrderInterface $order, $amount, $txn_type, $reference = NULL) {
    if ($txn_type == UC_CREDIT_PRIOR_AUTH_CAPTURE) {
      ...
      $nvp_request = [
        ...
        'FIRSTNAME' => substr($address->getFirstName(), 0, 25),
        'LASTNAME' => substr($address->getLastName(), 0, 25),
        'STREET' => substr($address->getStreet1(), 0, 100),
        'STREET2' => substr($address->getStreet2(), 0, 100),
      ...
      if ($order->isShippable()) {
        ...
        $nvp_request += [
          'SHIPTONAME' => substr($address->getFirstName() . ' ' . $address->getLastName(), 0, 25),
