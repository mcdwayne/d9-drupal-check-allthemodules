    ...
    switch ($operation) {
      case 'search':
        ...
        $query->fields('u', ['uid', 'mail'])
          ->fields('o', ['billing_first_name', 'billing_last_name'])
          ->condition('u.uid', 0, '>')
        ...
        if ($request->request->get('check') == TRUE) {
          ...
          if ($user_field_data = $result->fetchObject()) {
            ...
            if ($request->request->get('sendmail') == 'true') {
              ...
              \Drupal::service('plugin.manager.mail')->mail('user', 'register_admin_created', $email, uc_store_mail_recipient_langcode($email), ['account' => $account], uc_store_email_from());

              $build['email']['#markup'] .= $this->t('Account details sent to e-mail provided.<br /><br /><strong>Username:</strong> @username<br /><strong>Password:</strong> @password', ['@username' => $fields['name'], '@password' => $fields['pass']]) . '<br /><br />';
            }
