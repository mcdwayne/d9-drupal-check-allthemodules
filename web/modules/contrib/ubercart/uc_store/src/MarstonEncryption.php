  ...
  protected function applyFudgeFactor(&$fudgefactor) {
    ...
    if (!is_array($fudgefactor)) {
      ...
      if (!$alerted) {
        ...
        \Drupal::messenger()->addError(t('Ubercart cannot find a necessary encryption key. Refer to the store admin <a href=":url">dashboard</a> to isolate which one.', [':url' => Url::fromRoute('uc_store.admin')->toString()]));
