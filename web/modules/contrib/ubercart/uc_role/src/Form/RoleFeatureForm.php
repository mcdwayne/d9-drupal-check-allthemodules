...
class RoleFeatureForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, NodeInterface $node = NULL, $feature = NULL) {
    ...
    $form['role_lifetime'] = [
      ...
      '#states' => [
        'visible' => ['input[name="end_override"]' => ['checked' => TRUE]],
      ],
    ];
    ...
    $form['role_lifetime']['expire_relative_duration'] = [
      ...
      '#suffix' => '</div>',
      '#states' => [
        'visible' => ['select[name="expiration"]' => ['value' => 'rel']],
        'invisible' => ['select[name="expire_relative_granularity"]' => ['value' => 'never']],
      ],
    ];
    ...
    $form['role_lifetime']['absolute'] = [
      '#type' => 'container',
      '#states' => [
        'visible' => ['select[name="expiration"]' => ['value' => 'abs']],
      ],
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($product_role['end_override']) {
      if ($product_role['end_time']) {
        $description .= $this->t('<strong>Expiration:</strong> @date<br />', ['@date' => $this->dateFormatter->format($product_role['end_time'])]);
      }
      else {
