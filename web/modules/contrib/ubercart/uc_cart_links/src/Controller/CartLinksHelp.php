 ...
 *
 * @return array
 *   Form API render array with help text.
 */
class CartLinksHelp {
...
//  t('Specify the redirection by adding ?destination=url where url is the page to go to.'),
    ...
    $header = [t('Action'), t('Description'), t('Argument')];
    $rows = [
      ['p', t('Adds a product to the cart.'), t('A product node number, followed by optional arguments described in the table below.')],
      ['i', t('Sets the ID of the cart link.'), t('An alphanumeric string (32 characters max) to identify the link.')],
      ['m', t('Displays a message to the customer when the link is clicked.'), t('A <a href=":url">numeric message ID</a> to identify which message to display.', [':url' => Url::fromRoute('uc_cart_links.settings')->toString()])],
      ['e', t('Empties the cart. If used, this should be the first action.'), t('None.')],
    ];
    ...
    $header = [t('Argument'), t('Description'), t('Values')];
    $rows = [
      ['q', t('Specifies quantity of this product to add.'), t('A positive integer.')],
      ['a<aid>o<oid>', t('Specifies attribute/option for this product.'), t('aid is the integer attribute ID. oid is the integer option ID for radio, checkbox, and select options, or a url-escaped text string for textfield options.')],
      ['s', t('Silent.  Suppresses add-to-cart message for this product. (The add-to-cart message may be enabled on the <a href=":url">cart settings page</a>).', [':url' => Url::fromRoute('uc_cart.cart_settings')->toString()]), t('None.')],
    ];
    $build['args'] = [
