...
class Reports extends ControllerBase {
  ...
  public function customers() {
    ...
    $header = [
      ['data' => $this->t('#')],
      ['data' => $this->t('Customer'), 'field' => "ou.$last_name"],
      ['data' => $this->t('Username'), 'field' => "u.name"],
      ['data' => $this->t('Orders'), 'field' => 'orders'],
      ['data' => $this->t('Products'), 'field' => 'products'],
      ['data' => $this->t('Total'), 'field' => 'total', 'sort' => 'desc'],
      ['data' => $this->t('Average'), 'field' => 'average'],
    ];
    $csv_rows[] = [$this->t('#'), $this->t('Customer'), $this->t('Username'), $this->t('Orders'), $this->t('Products'), $this->t('Total'), $this->t('Average')];
    ...
    $query->fields('u', ['uid', 'name'])
      ->fields('ou', [$first_name, $last_name])
      ->condition('u.uid', 0, '>')
      ->groupBy('u.uid');
    ...
    foreach ($customers as $customer) {
      ...
      $rows[] = [
        ...
        ['data' => $name],
        ['data' => Link::createFromRoute($customer_name, 'entity.user.canonical', ['user' => $customer['uid']])->toString()],
        ['data' => $orders],
        ['data' => $products],
        ['data' => $total_revenue],
        ['data' => $average_revenue],
      ];
      $csv_rows[] = [$customer_number, $customer_order_name, $customer_name, $orders, $products, $customer['total'], $customer['average']];
    }
  ...
  public function products() {
    ...
    $header = [
      ['data' => $this->t('#')],
      ['data' => $this->t('Product'), 'field' => 'n.title'],
      ['data' => $this->t('Sold'), 'field' => 'sold'],
      ['data' => $this->t('Revenue'), 'field' => 'revenue', 'sort' => 'desc'],
      ['data' => $this->t('Gross'), 'field' => 'gross'],
    ];
    $csv_rows[] = [$this->t('#'), $this->t('Product'), $this->t('Sold'), $this->t('Revenue'), $this->t('Gross')];

    if ($views_column) {
      $header[] = ['data' => $this->t('Views'), 'field' => 'nc.totalcount'];
    ...
    foreach ($products as $product) {
      ...
      $row = [
        'data' => [
          $row_cell,
          $product_cell,
          ['data' => ['#prefix' => '<strong>', '#markup' => $sold_cell, '#suffix' => '</strong>']],
          ['data' => ['#prefix' => '<strong>', '#markup' => $revenue_cell, '#suffix' => '</strong>']],
          ['data' => ['#prefix' => '<strong>', '#markup' => $gross_cell, '#suffix' => '</strong>']],
        ],
        'primary' => TRUE,
      ];
      $csv_row = [$row_cell, $product_csv, $sold_csv, $revenue_csv, $gross_csv];
      ...
      if ($this->moduleHandler()->moduleExists('uc_attribute')) {
        ...
        foreach ($models as $model) {
          ...
          $breakdown_product = ['#markup' => "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$model"];
          $product_csv = "     $model";
          ...
          $breakdown_revenue = ['#theme' => 'uc_price', '#price' => $revenue_csv];

          $gross_csv = !empty($gross) ? $gross : 0;
          $breakdown_gross = ['#theme' => 'uc_price', '#price' => $gross_csv];
          ...
          $csv_row = ['', $product_csv, $sold_csv, $revenue_csv, $gross_csv];
  ...
  public function customProducts() {
    ...
    foreach ($products as $product) {
      ...
      if ($views_column) {
        ...
        $csv_rows[] = [$row_cell, $product_csv, $sold_csv, $revenue_csv, $gross_csv, $views];
      }
      else {
        $rows[] = [
          ['data' => $row_cell],
        ...
        $csv_rows[] = [$row_cell, $product_csv, $sold_csv, $revenue_csv, $gross_csv];
      }
    }
    $csv_data = $this->store_csv('uc_products', $csv_rows);
  ...
  public function sales() {
    ...
    $header = [$this->t('Sales data'), $this->t('Number of orders'), $this->t('Total revenue'), $this->t('Average order')];
    ...
    $rows[] = [
      ...
      $today['total'],
      ['data' => ['#theme' => 'uc_price', '#price' => $today['income']]],
      ['data' => ['#theme' => 'uc_price', '#price' => $today['average']]],
    ];
    ...
    $rows[] = [
      ...
      $yesterday['total'],
      ['data' => ['#theme' => 'uc_price', '#price' => $yesterday['income']]],
      ['data' => ['#theme' => 'uc_price', '#price' => $yesterday['average']]],
    ];
    ...
    $rows[] = [
      ...
      $month['total'],
      ['data' => ['#theme' => 'uc_price', '#price' => $month['income']]],
      ['data' => ['#theme' => 'uc_price', '#price' => $month['average']]],
    ];
    ...
    $rows[] = [
      ...
      $daily_orders,
      ['data' => ['#theme' => 'uc_price', '#price' => $daily_revenue]],
      '',
    ];
    ...
    $rows[] = [
      ...
      ['data' => ['#theme' => 'uc_price', '#price' => round($month['income'] + ($daily_revenue * $remaining_days), 2)]],
      '',
    ];
    ...
    $build['sales'] = [
      ...
      '#header' => $header,
      '#rows' => $rows,
      '#attributes' => ['class' => ['uc-sales-table']],
    ];

    // Build the header statistics table header.
    $header = [['data' => $this->t('Statistics'), 'width' => '50%'], ''];

    $rows = [
      [['data' => $this->t('Grand total sales')], ['data' => ['#theme' => 'uc_price', '#price' => db_query("SELECT SUM(order_total) FROM {uc_orders} WHERE order_status IN (:statuses[])", [':statuses[]' => $order_statuses])->fetchField()]]],
      [['data' => $this->t('Customers total')], ['data' => db_query("SELECT COUNT(DISTINCT uid) FROM {uc_orders} WHERE order_status IN (:statuses[])", [':statuses[]' => $order_statuses])->fetchField()]],
      [['data' => $this->t('New customers today')], ['data' => db_query("SELECT COUNT(DISTINCT uid) FROM {uc_orders} WHERE order_status IN (:statuses[]) AND :start <= created AND created <= :end", [':statuses[]' => $order_statuses, ':start' => $today_start, ':end' => $today_end])->fetchField()]],
      [['data' => $this->t('Online customers')], ['data' => db_query("SELECT COUNT(DISTINCT s.uid) FROM {sessions} s LEFT JOIN {uc_orders} o ON s.uid = o.uid WHERE s.uid > 0 AND o.order_status IN (:statuses[])", [':statuses[]' => $order_statuses])->fetchField()]],
    ];
    ...
    $header = [['data' => $this->t('Total orders by status'), 'width' => '50%'], ''];
    $rows = [];
    $unknown = 0;
...
*/
  ...
  public function yearSales() {
    ...
    $header = [$this->t('Month'), $this->t('Number of orders'), $this->t('Total revenue'), $this->t('Average order')];

    // Build the header to the CSV export.
    $csv_rows = [[$this->t('Month'), $this->t('Number of orders'), $this->t('Total revenue'), $this->t('Average order')]];
  ...
  public function customSales() {
    ...
    // Build the header for the report table.
    $header = [$this->t('Date'), $this->t('Number of orders'), $this->t('Products sold'), $this->t('Total revenue')];

    // Build the header to the CSV export.
    $csv_rows = [[$this->t('Date'), $this->t('Number of orders'), $this->t('Products sold'), $this->t('Total revenue')]];
