...
class AttributeTest extends UbercartBrowserTestBase {
  ...
  public function testAttributeApi() {
    ...
    $this->assertEquals($attribute->aid, db_query('SELECT aid FROM {uc_product_attributes} WHERE nid = :nid', [':nid' => $product->id()])->fetchField(), 'Attribute was attached to a product properly.');
    ...
    // Confirm the database is correct.
    $this->assertEquals($attribute->aid, db_query('SELECT aid FROM {uc_class_attributes} WHERE pcid = :pcid', [':pcid' => $product_class->id()])->fetchField(), 'Attribute was attached to a product class properly.');
    ...
    $this->assertEquals(count($aids), count($loaded_attributes), 'Verifying attribute result.');
    $this->assertEquals(count($aids), count(array_intersect_key($aids, $loaded_attributes)), 'Verifying attribute result.');
    ...
    $loaded_product_attributes = uc_attribute_load_multiple([], 'product', $product->id());

    // We'll get all in $loaded_attributes above, plus the original.
    $product_attributes = $loaded_attributes;

    // Make sure we only got the attributes we asked for. No more, no less.
    $this->assertEquals(count($loaded_product_attributes), count($product_attributes), 'Verifying attribute result.');
    $this->assertEquals(count($loaded_product_attributes), count(array_intersect_key($loaded_product_attributes, $product_attributes)), 'Verifying attribute result.');
    ...
    foreach ($all_options as $aid => $options) {
      foreach ($options as $oid => $option) {
        if (empty($loaded_product_attributes[$aid]) ||
            empty($loaded_product_attributes[$aid]->options[$oid])) {
          continue;
        }
    ...
    // Test loading all product attributes. (This covers
    // uc_attribute_load_product_attributes(), as the semantics are the same).
    $loaded_class_attributes = uc_attribute_load_multiple([], 'class', $product_class->id());

    // We'll get all in $loaded_attributes above, plus the original.
    $class_attributes = $loaded_attributes;

    // Make sure we only got the attributes we asked for. No more, no less.
    $this->assertEquals(count($loaded_class_attributes), count($class_attributes), 'Verifying attribute result.');
    $this->assertEquals(count($loaded_class_attributes), count(array_intersect_key($loaded_class_attributes, $class_attributes)), 'Verifying attribute result.');
    ...
    foreach ($all_options as $aid => $options) {
      foreach ($options as $oid => $option) {
        if (empty($loaded_class_attributes[$aid]) ||
            empty($loaded_class_attributes[$aid]->options[$oid])) {
          continue;
        }
    ...
    $this->createProductAdjustment(['combination' => 'a:1:{i:1;s:1:"1";}', 'nid' => 1]);
    $this->createProductAdjustment(['combination' => 'a:1:{i:1;s:1:"2";}', 'nid' => 1]);
    $this->createProductAdjustment(['combination' => 'a:1:{i:1;s:1:"3";}', 'nid' => 1]);
    $this->createProductAdjustment(['combination' => 'a:1:{i:2;s:1:"1";}', 'nid' => 2]);
    $this->createProductAdjustment(['combination' => 'a:1:{i:3;s:1:"1";}', 'nid' => 2]);
    $this->createProductAdjustment(['combination' => 'a:1:{i:1;s:1:"2";}', 'nid' => 3]);
    $this->createProductAdjustment(['combination' => 'a:1:{i:1;s:1:"3";}', 'nid' => 3]);
    $this->createProductAdjustment(['combination' => 'a:1:{i:3;s:1:"2";}', 'nid' => 3]);
    $this->createProductAdjustment(['combination' => 'a:1:{i:3;s:1:"3";}', 'nid' => 4]);

    // Test deletion by nid.
    uc_attribute_adjustments_delete(['nid' => 1]);
    $this->assertEquals(6, db_query('SELECT COUNT(*) FROM {uc_product_adjustments}')->fetchField());

    // Test deletion by aid.
    uc_attribute_adjustments_delete(['aid' => 2]);
    $this->assertEquals(5, db_query('SELECT COUNT(*) FROM {uc_product_adjustments}')->fetchField());

    // Test deletion by oid.
    uc_attribute_adjustments_delete(['oid' => 2]);
    $this->assertEquals(3, db_query('SELECT COUNT(*) FROM {uc_product_adjustments}')->fetchField());

    // Test deletion by aid and oid.
    uc_attribute_adjustments_delete(['aid' => 1, 'oid' => 3]);
    $this->assertEquals(2, db_query('SELECT COUNT(*) FROM {uc_product_adjustments}')->fetchField());
  }
  ...
  public function testAttributeUiSettings() {
    ...
    foreach (['none', 'adjustment', 'total'] as $type) {
  ...
  public function testAttributeUiAttributeOptionsAdd() {
    ...
    $this->drupalPostForm('admin/store/products/attributes/' . $attribute->aid . '/options/add', $edit, 'Submit');
  ...
  public function testAttributeUiAttributeOptionsEdit() {
    ...
    $edit = (array) $this->createAttributeOption(['aid' => $attribute->aid], FALSE);
    unset($edit['aid']);
    $this->drupalPostForm('admin/store/products/attributes/' . $attribute->aid . '/options/' . $option->oid . '/edit', $edit, 'Submit');
  ...
  public function testAttributeUiProductAdjustments() {
    ...
    for ($i = 0; $i < 3; $i++) {
      $option = $this->createAttributeOption(['aid' => $attribute->aid]);
      $adjustment = $this->createProductAdjustment(['combination' => serialize([$attribute->aid => $option->oid]), 'nid' => $product->id()]);
  ...
  public function testAttributeAddToCart() {
    ...
    for ($display = 0; $display <= 3; ++$display) {
      ...
      if ($display == 3) {
        $edit = ["attributes[$attribute->aid][$option->oid]" => $option->oid];
      }
      elseif (isset($option)) {
        $edit = ["attributes[$attribute->aid]" => $option->oid];
      }
      else {
        $option = new \stdClass();
        $option->name = self::randomMachineName();
        $option->price = 0;
        $edit = ["attributes[$attribute->aid]" => $option->name];
      }
  ...
  protected function attributeFieldsToTest($type = '') {
    ...
    switch ($type) {
      ...
      case 'class':
        ...
        $fields = array_merge($fields, [$info['id']]);
        break;
    }
    return $fields;
  }
