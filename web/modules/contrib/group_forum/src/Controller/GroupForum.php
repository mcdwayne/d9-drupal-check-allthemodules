...
/**
 * Returns responses for 'group_forum' GroupContent routes.
 */
class GroupForum extends GroupContentController {
    ...
    foreach ($this->addPageBundles($group, $create_mode) as $plugin_id => $bundle_name) {
      if (!empty($build['#bundles'][$bundle_name])) {
        ...
        $description = $create_mode
          ? $this->t('Create a forum container in the group.')
          : $this->t('Add an existing forum container to the group.');
