...
class ProblemLog {
   ...
   * @see db_insert()
   */
  public function log($donation_uuid, $type, $detail, $user_agent = NULL) {
    $return_value = NULL;
    $entry = [
  ...
  public static function load($donation_uuid) {
    return db_query('SELECT type, detail, user_agent, timestamp FROM {give_problem} WHERE donation_uuid = :donation_uuid', [':donation_uuid' => $donation_uuid])->fetchAll();
  }
