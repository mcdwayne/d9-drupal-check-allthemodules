...
class Vocabulary extends DrupalSqlBase {
  ...
  public function prepareRow(Row $row) {
    // Port from 'Course categories' to 'Learning path category'.
    $machine_name = $row->getSourceProperty('machine_name');
    if ($machine_name == 'course_categories') {
      $row->setSourceProperty('machine_name', 'learning_path_category');
      $row->setSourceProperty('name', t('Learning path category'));
    }
