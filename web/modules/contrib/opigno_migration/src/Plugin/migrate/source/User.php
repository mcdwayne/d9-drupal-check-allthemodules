...
class User extends FieldableEntity {
  ...
  public function prepareRow(Row $row) {
    ...
    $old_roles = $this->select('users_roles', 'ur')
      ->fields('ur', ['rid'])
    ...
    // Find porting users with roles 'student manager'
    // and set them new roles 'user manager',
    // 'statistics reader' and 'content manager'.
    // TODO: Needs to do something with role 'forum administrator'.
    foreach ($old_roles as $old_role) {
      if (in_array($old_role, ['5' => '5'])) {
        $roles = $new_roles;
      }
    }
