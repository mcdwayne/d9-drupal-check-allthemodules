...
class SiteSettingReplicateForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $setting = FALSE) {
    ...
    if (!$has_replicate && !$has_field_tools) {
      drupal_set_message($this->t('Please install the Replicate and Field Tools modules to use this feature.'), 'warning');
    }
    elseif (!$has_replicate) {
      drupal_set_message($this->t('Please install the Replicate module to use this feature.'), 'warning');
    }
    elseif (!$has_field_tools) {
      drupal_set_message($this->t('Please install the Field Tools module to use this feature.'), 'warning');
    }
    elseif ($site_setting_entity_type = $this->entityTypeManager->getStorage('site_setting_entity_type')->load($setting)) {
      ...
      drupal_set_message($this->t('Unable to load setting.', 'warning'));
    }
    ...
    if ($existing = $this->entityTypeManager->getStorage('site_setting_entity_type')->loadMultiple()) {
      ...
      foreach ($values['new_settings'] as $key => $setting) {
        ...
        if (empty($setting['machine_name'])) {
          $form_state->setErrorByName('new_settings][' . $key . '][machine_name', t('Please enter a machine name'));
        }
        if (empty($setting['label'])) {
          $form_state->setErrorByName('new_settings][' . $key . '][label', t('Please enter a label'));
        }
        if (empty($setting['fieldset'])) {
          $form_state->setErrorByName('new_settings][' . $key . '][fieldset', t('Please enter a fieldset'));
        }
