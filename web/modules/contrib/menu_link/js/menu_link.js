  ...
  Drupal.behaviors.menuLinkDetailsSummaries = {
    attach: function (context) {
      $(context).find('.menu-link-form').drupalSetSummary(function (context) {
        var $context = $(context);
        if ($context.find('.js-form-item-menu-link-0-enabled input').is(':checked')) {
          return Drupal.checkPlain($context.find('.js-form-item-menu-link-0-title input').val());
        }
        else {
  ...
  Drupal.behaviors.menuLinkLinkAutomaticTitle = {
    attach: function (context) {
      ...
      $context.find('.menu-link-form').each(function () {
        ...
        // alterations.
        var $checkbox = $this.find('.js-form-item-menu-link-0-enabled input');
        var $link_title = $context.find('.js-form-item-menu-link-0-title input');
