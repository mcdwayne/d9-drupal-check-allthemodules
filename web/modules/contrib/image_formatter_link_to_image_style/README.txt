DESCRIPTION
-----------

This module provides an additional formatter for image core field,
to link to an image style.


INSTALLATION
------------

1) Place this module directory in your "modules" folder (this will usually be
   "modules/"). Don't install your module in Drupal core's "modules"
   folder, since that will cause problems and is bad practice in general. If
   "modules" doesn't exist yet, just create it.

2) Enable the module.


SPONSORS
--------

 * This Drupal 7 module is sponsored by NovaRosa.es ~ http://novarosa.es
 * This Drupal 8 module is sponsored by NovaRosa.es ~ http://novarosa.es


AUTHOR
------

Manuel García (facine) ~ http://facine.es
