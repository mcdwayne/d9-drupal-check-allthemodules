...
class XmlEncoder extends SerializerAwareEncoder implements EncoderInterface, DecoderInterface, NormalizationAwareInterface
{
     ...
     */
    public function __construct($rootNodeName = 'data', $comments = [])
    {
    ...
    {
        // Erase all comments because Encoder doesn't respect them.
        $data = preg_replace('/\<\!\-\-.*(\n)?(\r)?-->/U', '', $data);
        ...
        if ($rootNode->hasChildNodes()) {
            ...
            if (empty($data)) {
                $temp = $this->parseXml($rootNode);
                return $temp;
            }
            $temp = array_merge($data, (array) $this->parseXml($rootNode));
            return $temp;
        }
