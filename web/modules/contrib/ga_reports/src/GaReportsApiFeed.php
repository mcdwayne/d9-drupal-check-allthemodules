...
/**
 * Class GaReportsApiFeed.
 *
 * GaReportsApiFeed class to authorize access to and request data
...
class GaReportsApiFeed {
  ...
  public function query($url, $params = [], $method = 'GET', $headers, $cache_options = []) {
    ...
    if (empty($cache_options['cid'])) {
      $cache_options['cid'] = 'ga_reports_data:' . md5(serialize(array_merge($params, [$url, $method])));
    }
