...
/**
 * @file
 * Contains \Drupal\rut_field\Plugin\field\field_type\RutItem.
 */
...
class RutItem extends FieldItemBase {
  ...
  public function setValue($value, $notify = TRUE) {
    ...
    elseif (is_array($value) && !isset($value['value']) && isset($value['rut']) && isset($value['dv'])) {
      $value['value'] = RutUtil::formatterRut($value['rut'], $value['dv']);
  ...
  public function onChange($property_name, $notify = TRUE) {
    if ($property_name == 'value') {
      list($rut, $dv) = RutUtil::separateRut($this->get('value')->getString());
      $this->writePropertyValue('rut', $rut);
      $this->writePropertyValue('dv', $dv);
    }
    elseif ($property_name == 'rut' || $property_name == 'dv') {
      $rut_complete = RutUtil::formatterRut($this->get('rut')->getString(), $this->get('dv')->getString());
