 ...
 */
class SimplySignupsFieldsSelectForm extends FormBase {
  ...
  public function getFormId() {
    return 'simply_signups_fields_select_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['#attributes'] = [
      'class' => ['simply-signups-fields-select-form', 'simply-signups-form'],
    ];
    $form['request'] = [
      '#type' => 'hidden',
      '#value' => $request,
    ...
    $form['options'] = [
      ...
      '#description' => $this->t('The possible values this field can contain. Enter one value per line, in the format key|label.<br />The key is the stored value. The label will be used in displayed values and edit forms.<br />The label is optional: if a line contains a single string, it will be used as key and label.'),
      '#required' => TRUE,
      '#default_value' => isset($field['#options']) ? SimplySignupsUtility::sanitizeOptionsTextarea($field['#options']) : '',
    ];
    $form['multiple'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Multiple select'),
      '#return_value' => 1,
      '#description' => $this->t('Indicates whether one or MORE options can be selected.'),
      '#default_value' => (isset($field['#multiple'])) ? $field['#multiple'] : 0,
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $textareaOptions = $values['options'];
    $sanitizedOptions = SimplySignupsUtility::sanitizeOptionsSelect($textareaOptions, $values['required']);
    $field = [
      '#type' => $values['type'],
      '#title' => $values['machine_name'],
      '#options' => $sanitizedOptions,
      '#required' => ($values['required'] == 1) ? 1 : 0,
      '#multiple' => ($values['multiple'] == 1) ? 1 : 0,
      '#description' => (!empty($values['description'])) ? $values['description'] : '',
      '#default_value' => (!empty($values['default_value'])) ? $values['default_value'] : NULL,
    ];
    $db = $this->database;
