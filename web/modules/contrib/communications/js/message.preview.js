...
(function ($, Drupal) {
  Drupal.behaviors.messagePreviewDestroyLinks = {
    attach: function attach(context) {
      function clickPreviewModal(event) {
        if (event.button === 0 && !event.altKey && !event.ctrlKey && !event.metaKey && !event.shiftKey) {
          event.preventDefault();
          var $previewDialog = $('<div>' + Drupal.theme('messagePreviewModal') + '</div>').appendTo('body');
      ...
      var $preview = $(context).once('message-preview');
      if ($(context).find('.message-preview-container').length) {
        $preview.on('click.preview', 'a:not([href^="#"], .message-preview-container a)', clickPreviewModal);
      }
    },
    detach: function detach(context, settings, trigger) {
      if (trigger === 'unload') {
        var $preview = $(context).find('.content').removeOnce('message-preview');
        if ($preview.length) {
  ...
  Drupal.behaviors.messagePreviewSwitchViewMode = {
    attach: function attach(context) {
  ...
  Drupal.theme.messagePreviewModal = function () {
