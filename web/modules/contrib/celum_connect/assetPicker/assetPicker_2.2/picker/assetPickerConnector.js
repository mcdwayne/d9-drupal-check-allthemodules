...
Celum.AssetPickerConnector = function () {
  ...
  const locale = util.getFrameProperty('clm-locale');
  ...
  return {
    ...
    applyCss: function () {
      const css = util.getFrameProperty('clm-css') || null;
    ...
    applyJs: function () {
      const js = util.getFrameProperty('clm-js') || null;
    ...
    fireEvent: function (data) {
      ...
      const parentOrigin = util.getFrameProperty('clm-origin');
      ...
      if (event.origin === parentOrigin) {
        ...
        if (util.getFrameProperty('id') == data.target) {
          if (typeof data.action === 'string') {
            const scope = angular.element($(".actionbar-content-wrapper")).scope();
    ...
    handleIe10Resize: function () {
      ...
      if ($('body').hasClass('ie10')) {
        window.setTimeout(function () {
          const h = $('.appContentView').height() - $('.filter-box').height() - $('.controls-box').height() - 30;
          $('.list-box').height(h);
        }, 1000);
      }
    }
