...
namespace Drupal\commerce_pagseguro_transp\BundlePlugin;
  ...
  public function validate($module) {
    ...
    foreach (entity_get_bundle_plugin_entity_types() as $entity_type) {
      ...
      if (!empty($bundles_filtered_by_module)) {
        ...
        foreach ($bundles_with_content as $bundle_id => $bundle) {
          $reasons[] = $this->t('Bruno: There is data for the bundle @bundle on the entity type @entity_type. <a href=":url">Remove @entity_type_plural</a>.', [
            '@bundle' => $bundle['label'],
            '@entity_type' => $entity_type->getLabel(),
            '@entity_type_plural' => $entity_type->getPluralLabel(),
            ':url' => Url::fromRoute('commerce_pagseguro_transp.prepare_modules_entity_uninstall', ['entity_type_id' => $entity_type->id(), 'bundle_id' => $bundle_id])->toString(),
          ]);
        }
      }
    }
