...
(function ($, Modernizr, Drupal) {
  ...
  function CollapsibleDetails(node) {
    ...
    // Init classes for IE8.
    this.$node.toggleClass('collapse-closed', this.$node.attr('open') !== 'open');
    // Expand details if there are errors inside, or if it contains an
    // element that is targeted by the URI fragment identifier.
    var anchor = location.hash && location.hash !== '#' ? ', ' + location.hash : '';
    if (this.$node.find('.error' + anchor).length) {
      this.$node.attr('open', true).removeClass('collapse-closed');
    }
  ...
  $.extend(CollapsibleDetails.prototype, /** @lends Drupal.CollapsibleDetails# */{
    ...
    toggle: function () {
      ...
      setTimeout(function () {
        this.$node.attr('open', !isOpen).toggleClass('collapse-closed', isOpen);
      }.bind(this), 0);
    }
  });
