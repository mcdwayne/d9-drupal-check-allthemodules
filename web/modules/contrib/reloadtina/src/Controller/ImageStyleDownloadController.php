...
namespace Drupal\reloadtina\Controller;

use Drupal\image\Controller\ImageStyleDownloadController as ImageController;
 ...
 */
class ImageStyleDownloadController extends ImageController {
  ...
  public function deliver(Request $request, $scheme, ImageStyleInterface $image_style) {
    ...
    $multiplier = isset($image_style->multiplier) ? (float) $image_style->multiplier : 1.0;

    if ($multiplier != 1 && !in_array($multiplier, $multipliers)) {
    ...
    $derivative_uri = reloadtina_build_uri($image_style, $image_uri, $multiplier);
    $headers = array();
    ...
    if (file_exists($derivative_uri)) {
      ...
      if ($multiplier != 1) {
        reloadtina_apply_multiplication_to_effects($image_style, $multiplier);
      }
