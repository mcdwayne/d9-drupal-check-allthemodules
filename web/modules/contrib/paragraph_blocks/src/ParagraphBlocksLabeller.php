...
class ParagraphBlocksLabeller {
  ...
  public function getTitle($plugin_id, &$enabled) {
    ...
    list($plugin_type_id, $plugin_entity_type_id, $plugin_field_name, $plugin_field_delta, $plugin_field_bundle) = $plugin_parts;
    if ($plugin_type_id != self::PLUGIN_TYPE_ID) {
      return NULL;
    }

    // Only check the field bundle if it exists. This is new to the 2.x branch.
    // So this check exists for backwards compatability.
    if ($plugin_field_bundle) {
      // Remove if this paragraph field if it is not enabled.
