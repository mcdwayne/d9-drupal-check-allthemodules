(function($) {
  Drupal.behaviors.tipsy = {
    attach: function(context, settings) {
      ...
      if (settings.tipsy.drupal_forms.forms) {
        var formElement = $('.form-item');
        formElement.each(function(){
          var desc = $(this).find('.description');
          desc.css('display', 'none');
          if(desc.length > 0) {
            formSettings = settings.tipsy.drupal_forms.options;
      ...
      if (settings.tipsy.custom_selectors) {
        var selectors = settings.tipsy.custom_selectors;
