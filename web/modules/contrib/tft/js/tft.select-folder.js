...
Drupal.behaviors.initTFTNodeForm = {
  ...
  attach: function(context) {
    ...
    $('li.parent-folder span.closed-icon').each(function() {
      (function ($this) {
        $this.once('tft_open_folder_click').click(function(e) {
    ...
    $('a.folder-link').each(function() {
      (function($this) {
        $this.once('tft_folder_click').click(function(e) {
    ...
    setActiveFolder($('select[name="tft_folder"]').val(), true);
  }
}
...
function setActiveFolder(tid, scroll) {
  ...
  $('select[name="tft_folder"]').val(tid);
