...
class Question extends ContentEntityBase implements QuestionInterface {
  ...
  public function addReport($user_id) {
    ...
    $reported = $this->getReports();
    // Add this user.
    $reported[] = $user_id;
    $this->reported->setValue([
      'reported' => $reported,
    ]);
    $this->reported_count = count($reported);
    $this->save();
  }
  ...
  public function getSubscribed() {
    ...
    if (!empty($subscriptions)) {
      $subscriptions = $subscriptions->getValue();
      if (isset($subscriptions['subscribed'])) {
