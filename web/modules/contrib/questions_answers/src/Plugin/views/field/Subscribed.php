...
/**
 * Field handler to display subscribed user count.
 *
 * @ingroup views_field_handlers
 *
 * @ViewsField("questions_answers_subscribed")
 */
class Subscribed extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    ...
    switch ($this->options['display_type']) {
      ...
      case 'flag':
        return $entity->getSubscribedCount() > 0 ? $this->t('Yes') : $this->t('No');

      case 'list':
        if ($entity->getSubscribedCount() > 0) {
          $users = [
            '#theme' => 'item_list',
            '#items' => [],
            '#type' => 'ul',
            '#title' => $this->t('Subscribed to by'),
          ];
          foreach ($entity->getSubscribed() as $uid) {
            $user = User::load($uid);
      ...
      case 'count':
        if ($entity->getSubscribedCount() > 0) {
          return $this->t('Subscribed to by @count users', [
            '@count' => $entity->getSubscribedCount(),
          ]);
        }
        break;
    }

    return $this->t('No subscriptions');
  }
