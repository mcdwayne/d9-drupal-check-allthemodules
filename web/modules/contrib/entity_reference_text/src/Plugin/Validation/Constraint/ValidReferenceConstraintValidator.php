...
/**
 * Implements a entity_reference_text specific validator.
 */
class ValidReferenceConstraintValidator extends ConstraintValidator implements ContainerInjectionInterface {
  ...
  public function validate($value, Constraint $constraint) {
    ...
    $new_entities = [];
    $target_ids = [];
    foreach ($value->entity_ids as $delta => $target_id) {
      ...
      if ($target_id === NULL) {
        if ($value->entities[$target_id]->isNew()) {
          $this->context->buildViolation($constraint->nullMessage)
            ->atPath('entity_ids')
            ->addViolation();
          return;
        }
        $new_entities[$delta] = $value->entities[$target_id];
      }
