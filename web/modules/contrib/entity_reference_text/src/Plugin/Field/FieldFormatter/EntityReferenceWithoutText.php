 ...
 *
 * @FieldFormatter(
 *   id = "entity_reference_without_text",
 *   label = @Translation("Entity reference without text"),
 *   field_types = {
 *     "entity_references_text",
 *   },
 * )
 */
class EntityReferenceWithoutText extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['limit'] = [
      '#title' => t('Maximum amount of items'),
      '#type' => 'textfield',
  ...
  protected function doViewElement(FieldItemInterface $item, $langcode, $output_as_link) {
    ...
    if ($this->getSetting('limit')) {
      $entities = array_slice($entities, 0, $this->getSetting('limit'));
    }
    ...
    foreach ($entities as $entity) {
      ...
      if ($output_as_link && !$entity->isNew()) {
        try {
          $element[] = $entity->toLink()->toRenderable();
        }
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($items as $delta => $item) {
      $elements[] = $this->doViewElement($item, $langcode, $output_as_link);
    }
    return $elements;
  }
