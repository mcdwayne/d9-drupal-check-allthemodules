      ...
      foreach($templates as $template) {
        foreach($template->tasks as $task) {
          if($task['tasktype'] == 'MaestroNonIntExample') {
            $reasons[] = $this->t('To uninstall the Non Interactive Plugin Task Example module, remove the Non Interactive Example task from the <em>:template</em> template.', array(':template' => $template->label));
          }
        }
      }
    }
