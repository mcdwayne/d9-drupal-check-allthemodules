 ...
 */
class EntityDomainAccessSettings extends FormBase {
   ...
   *   The domain entity mapper.
   */
  public function __construct(EntityTypeManagerInterface $entity_type_manager, EntityTypeBundleInfoInterface $entity_type_bundle_info, DomainLoaderInterface $domain_loader, EntityDomainAccessMapper $mapper) {
  ...
  public function getFormId() {
    return 'entity_domain_access_settings';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $entity_type_id = NULL) {
    ...
    $domain_entity_behavior = [
      EntityDomainAccessMapper::BEHAVIOR_AUTO => $this->t('Affiliate automatically created entity to a value (no widget on entity creation form, auto-assignation)'),
      EntityDomainAccessMapper::BEHAVIOR_USER => $this->t('User choose affiliate, with a default value (form widget on the entity creation form)'),
    ];
    ...
    foreach ($bundles as $bundle_id => $bundle) {
      ...
      $settings = [];
      if ($field = $this->mapper->loadField($entity_type_id, $bundle_id, EntityDomainAccessMapper::FIELD_NAME)) {
        $settings = $field->getThirdPartySettings('entity_domain_access');
        $has_fields = TRUE;
      }
      $settings += [
        'domains' => [],
        'behavior' => EntityDomainAccessMapper::BEHAVIOR_AUTO,
      ];
      $form[$bundle_id] = [
        '#type' => 'details',
        '#title' => $bundle_label,
      ...
      $states_domains = $states;
      $states_domains['visible']["select[name=\"{$bundle_id}_behavior\"]"] = ['value' => EntityDomainAccessMapper::BEHAVIOR_USER];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($bundles as $bundle) {
      if (empty($values[$bundle . '_enable'])) {
        if ($field_domain_access = $this->mapper->loadField($entity_type_id, $bundle, EntityDomainAccessMapper::FIELD_NAME)) {
          $field_domain_access->delete();
        }
        if ($field_domain_all = $this->mapper->loadField($entity_type_id, $bundle, EntityDomainAccessMapper::FIELD_ALL_NAME)) {
          $field_domain_all->delete();
        }
      }
      else {
        if ($field_domain_access = $this->mapper->loadField($entity_type_id, $bundle, EntityDomainAccessMapper::FIELD_NAME)) {
          // Update settings.
          $field_domain_access->setThirdPartySetting('entity_domain_access', 'domains', array_filter($values[$bundle . '_domains']));
          $field_domain_access->setThirdPartySetting('entity_domain_access', 'behavior', $values[$bundle . '_behavior']);
          $field_domain_access->save();
          if ($field_domain_all = $this->mapper->loadField($entity_type_id, $bundle, EntityDomainAccessMapper::FIELD_ALL_NAME)) {
            // Update settings.
            $field_domain_all->setThirdPartySetting('entity_domain_access', 'behavior', $values[$bundle . '_behavior']);
            $field_domain_all->save();
          }
          else {
