...
use Drupal\cleverreach\Exception\MethodNotAllowedException;
...
class CleverreachSearchController {
   ...
   *
   * @param \Symfony\Component\HttpFoundation\Request $request
   *   Request search parameters, currently supported: entity_id, title and language.
   *
   * @return array
  ...
  private function getSearchResult(Request $request) {
    ...
    if ($value = $request->get('title')) {
      $filter[] = ['field' => 'title', 'value' => $value, 'condition' => 'CONTAINS'];
    }
   ...
   *
   * @return \Drupal\cleverreach\Component\Repository\ArticleRepository
   */
