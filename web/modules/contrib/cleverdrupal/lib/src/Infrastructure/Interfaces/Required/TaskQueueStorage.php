...
interface TaskQueueStorage {
   ...
   *   List of key/value pairs that must be satisfied upon saving queue item. Key is queue item
   ...
   *
   * @return \CleverReach\Infrastructure\TaskExecution\QueueItem|null Found queue item or null when queue item does not exist
   */
   ...
   *   Task context restriction if provided search will be limited to given task context. Leave empty for
   *   search across all task contexts.
   *
   * @return \CleverReach\Infrastructure\TaskExecution\QueueItem|null Found queue item or null when queue item does not exist
   */
   ...
   *   List of simple search filters, where key is queue item property and value is condition
   *   value for that property. Leave empty for unfiltered result.
   * @param array $sortBy
   *   List of sorting options where key is queue item property and value sort direction ("ASC" or "DESC").
