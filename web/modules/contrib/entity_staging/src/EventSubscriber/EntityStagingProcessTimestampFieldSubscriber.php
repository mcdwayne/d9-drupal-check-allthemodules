 ...
 *
 * Get the migration definition for processing a timestamp field.
 */
class EntityStagingProcessTimestampFieldSubscriber implements EventSubscriberInterface {
  ...
  public function getProcessFieldDefinition(EntityStagingProcessFieldDefinitionEvent $event) {
    if (in_array($event->getFieldDefinition()->getType(), ['timestamp', 'changed', 'created'])) {
      $process_field = [
        'plugin' => 'callback',
        'callable' => 'strtotime',
