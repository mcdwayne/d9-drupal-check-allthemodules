    ...
    public function attributes($attributes, $elem = '') {
        ...
        foreach ($return as $name => $info) {
            ...
            if ($name == 'style') {
                ...
                $properties = array_filter(array_map('trim', explode(';', HtmlString::decodeEntities($info['value']))));
                foreach ($properties as $property) {
                    ...
                    if (strpos($property_value, 'url(') !== false) {
                        ...
                        if (HtmlString::filterBadProtocol($url) != $url) {
                            continue;
                        }
                        if (!preg_match('`^/[^/]+`', $url)) {
                            $match = false;
                    ...
                    $sanitized_properties[] = $property_name . ':' . HtmlString::checkPlain($property_value);
                }
                ...
                $info['value'] = HtmlString::filterBadProtocol($info['value']);
            }
