...
/**
 * @file
 * Contains \Drupal\xwechat_material\Form\MaterialAddOtherForm.
 */
 ...
 */
class MaterialAddOtherForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'xwechat_add_other_material';
  }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $file = file_save_upload('upload', $form['upload']['#upload_validators'], FALSE, 0);
    if (isset($file)) {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($values['upload'])) {
      $filename = file_unmanaged_copy($values['upload']->getFileUri());
    ...
    if (!empty($values['material_path'])) {
      ...
      $wechat->uploadImg(drupal_realpath($values['material_path']));
      //$wechat->getMaterialList(array('type'=>'news', 'offset' => 0, 'count' => 20));
    }
