...
class EventHandler {
  ...
  public static function userUpdated(EntityInterface $entity) {
    ...
    if ($fields = $entity->getFields(FALSE)) {
      ...
      if (!empty($changed)) {
        TaskQueue::enqueue(new RecipientDeactivateSyncTask($changed));
      }
    }

    TaskQueue::enqueue(new RecipientSyncTask([$entity->id()], [], FALSE));
  }
  ...
  public static function userDeleted(EntityInterface $entity) {
    ...
    if (!empty($emails)) {
      TaskQueue::enqueue(new RecipientDeactivateSyncTask($emails));
    }
  }
  ...
  public static function termCreated(EntityInterface $entity) {
    ...
    Logger::logInfo(
        'New newsletter taxonomy tag create event detected. Tag ID: ' .
        $entity->id(),
        'Integration'
    );
  ...
  public static function termUpdate(EntityInterface $entity, $label) {
    ...
    Logger::logInfo(
        'Newsletter taxonomy tag update event detected. Tag ID: ' .
        $entity->id(),
        'Integration'
    );
   ...
   *   Original site name.
   *
   * @see \Drupal\clever_reach\EventSubscriber\ConfigUpdateSubscriber
   *
   * @throws \CleverReach\Infrastructure\TaskExecution\Exceptions\QueueStorageUnavailableException
   */
  ...
  /**
   * Checks if execution of event is allowed.
   *
   * Event execution should be allowed only if initial sync is already done.
   *
   * @return bool
  ...
  private static function isUserTaxonomyTerm($vocabulary) {
    return in_array(
        $vocabulary,
        clever_reach_get_taxonomy_vocabularies(),
        TRUE
    );
  }
