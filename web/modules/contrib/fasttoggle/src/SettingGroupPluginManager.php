 ...
 *
 * The SettingGroupPluginManager class extends the DefaultPluginManager to
 * provide a way to manage setting group plugins.
 *
 * As well as this class definition, we need to declare our plugin manager
 * class as a service, in the plugin_type_example.services.yml file.
 */
class SettingGroupPluginManager extends DefaultPluginManager {
  ...
  public function __construct(\Traversable $namespaces, CacheBackendInterface $cache_backend, ModuleHandlerInterface $module_handler) {
    ...
    $plugin_interface = 'Drupal\fasttoggle\Plugin\SettingGroup\SettingGroupInterface';
    ...
    $this->alterInfo('fasttoggle_setting_group_info');
    ...
    $this->setCacheBackend($cache_backend, 'fasttoggle_setting_group_info');
  }
   ...
   *
   * @return \Drupal\fasttoggle\SettingGroupPlugin or NULL
   *   The matching plugin.
   */
  public function match($seeking) {
    ...
    foreach($definitions as $group => $definition) {
      $plugin = $this->createInstance($group);
      if ($plugin->matches($seeking)) {
        return $plugin;
      }
    }
