 ...
 *
 * The SettingsPluginManager class extends the DefaultPluginManager
 * to provide a way to manage Fastoggle's Settings plugins.
 *
 */
class SettingPluginManager extends DefaultPluginManager {
  ...
  public function __construct(\Traversable $namespaces, CacheBackendInterface $cache_backend, ModuleHandlerInterface $module_handler) {
    ...
    // This tells the plugin system to look for plugins in the 'Plugin/Setting'
    // subfolder inside modules' 'src' folder.
    $subdir = 'Plugin/Setting';
    ...
    $plugin_interface = 'Drupal\fasttoggle\Plugin\Setting\SettingInterface';
    ...
    $this->alterInfo('fasttoggle_setting_info');
    ...
    $this->setCacheBackend($cache_backend, 'fasttoggle_setting_info');
  }
   ...
   *
   * @return \Drupal\fasttoggle\SettingPlugin or NULL
   *   The matching plugin.
   */
  public function match($seeking) {
    ...
    foreach($definitions as $setting => $definition) {
      $plugin = $this->createInstance($setting);
