    ...
    attach: function (context, settings) {
      ...
      if ($("#simple-dialog-container").length == 0) {
        ...
        try {
          ...
          var defaultOptions = Drupal.simpleDialog.explodeOptions(settings.simpleDialog.defaults.settings);
        ...
        if (!event.metaKey && !$(this).hasClass('simpleDialogProcessed')) {
          ...
          $(this).click(function(event) {
            ...
            var url = $(this).attr('href');
      	    ...
      	    var title = $(this).attr('title') ? $(this).attr('title') : settings.simpleDialog.defaults.title;
      	    if (!title) {
      		    title = $(this).text();
      	    }
            // Use defaults if not provided
            var selector = $(this).attr('name') ? $(this).attr('name') : settings.simpleDialog.defaults.target_selector;
            var options = $(this).attr('rel') ? Drupal.simpleDialog.explodeOptions($(this).attr('rel')) : Drupal.simpleDialog.explodeOptions(settings.simpleDialog.defaults.settings);
