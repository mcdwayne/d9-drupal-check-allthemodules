...
class UrlRedirectForm extends EntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status) {
      drupal_set_message($this->t('Saved the %label UrlRedirect.', array(
            '%label' => $url_redirect->label(),
      )));
    }
    else {
      drupal_set_message($this->t('The %label UrlRedirect was not saved.', array(
            '%label' => $url_redirect->label(),
      )));
    }
  ...
  public function exist($id) {
    $entity = $this->entityQuery->get('url_redirect')
        ->condition('id', $id)
        ->execute();
    return (bool) $entity;
  }
