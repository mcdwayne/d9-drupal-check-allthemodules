...
/**
 * Plugin for numeric elevation formatters.
 *
 * @FieldFormatter(
 *   id = "number_elevation",
 *   label = @Translation("Elevation"),
 *   field_types = {
 *     "decimal",
 *     "float"
 *   }
 * )
 */
class ElevationFormatter extends DecimalFormatter {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['elevation_unit'] = [
      '#type' => 'select',
      '#title' => t('Elevation unit'),
      '#options' => $options,
      '#default_value' => $this->getSetting('elevation_unit'),
      '#weight' => 0,
    ];
  ...
  public function settingsSummary() {
    ...
    if ($this->getSetting('suffix_unit')) {
      $summary[] = t('Display with <em>@unit</em> as suffix.', ['@unit' => $this->getSetting('elevation_unit')]);
    }
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($items as $delta => $item) {
      ...
      if ($this->getSetting('suffix_unit')) {
        $suffix = $this->getSetting('elevation_unit');
  ...
  /**
   * Change the elevation in meters to the configured measurement unit.
   *
   * @param $value
   *
   * @return float|int
  ...
  private function convertUnit($value) {
    $unit = $this->getSetting('elevation_unit');
    switch ($unit) {
      case 'm':
      default:
        return $value;
