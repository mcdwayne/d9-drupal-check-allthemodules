  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $form['smartypants_enabled'] = array(
      ...
      '#title' => $this->t('Use typographers quotation marks and dashes (<a href="@smarty-pants-url">SmartyPants</a>)',
        array('@smarty-pants-url' => 'http://daringfireball.net/projects/smartypants')
      ),
    ...
    $form['smartypants_hyphens'] = array(
      '#type' => 'select',
      '#title' => $this->t('Hyphenation settings for SmartyPants'),
    ...
    $form['wrap_abbr'] = array(
      ...
      '#description' => $this->t('Wraps abbreviations with <code>@wrapper-code</code> and inserts space after the dots.', array('@wrapper-code' => '<span class="abbr">…</span>')),
    ...
    $form['wrap_numbers'] = array(
      ...
      '#description' => $this->t('Wraps numbers with <code>@wrapper-code</code> and inserts thin space for digit grouping.', array('@wrapper-code' => '<span class="number">…</span>')),
    ...
    foreach (UnicodeConversion::map('ligature') as $ascii => $unicode) {
      $ligature_options[$ascii] = $this->t('Convert <code>@ascii</code> to @unicode', array(
        '@ascii' => $ascii,
        '@unicode' => $unicode,
      ));
    }
    ...
    foreach (UnicodeConversion::map('arrow') as $ascii => $unicode) {
      $arrow_options[$ascii] = $this->t('Convert <code>@ascii</code> to @unicode', array(
    ...
    foreach (UnicodeConversion::map('fraction') as $ascii => $unicode) {
      $fraction_options[$ascii] = $this->t('Convert <code>@ascii</code> to @unicode', array(
        '@ascii' => $ascii,
        '@unicode' => $unicode,
      ));
    ...
    foreach (UnicodeConversion::map('quotes') as $quotes => $unicode) {
      $quotes_options[$quotes] = $this->t('Convert <code>@ascii</code> to @unicode', array(
    ...
    $version_strings[] = $this->t('SmartyPants PHP version: <a href="@smarty-pants-url">@version</a>', array(
      '@smarty-pants-url' => 'http://www.michelf.com/projects/php-smartypants/',
      '@version' => SmartyPants::SMARTYPANTS_PHP_VERSION,
    ));
    $version_strings[] = $this->t('PHP Typogrify Version: <a href="@php-typogrify-url">@version</a>', array(
      '@php-typogrify-url' => 'http://blog.hamstu.com/',
      '@version' => PHP_TYPOGRIFY_VERSION,
    ));
    ...
    if ($long) {
      ...
      if ($settings['wrap_ampersand']) {
        $output .= '<li>' . $this->t('Wraps ampersands (the “&amp;” character) with <code>@wrapper-code</code>.',
          array('@wrapper-code' => '<span class="amp">&</span>')) . '</li>';
      }
      if ($settings['widont_enabled']) {
      ...
      if ($settings['wrap_initial_quotes']) {
        ...
        $output .= '</li><li>' . $this->t('Wraps initial quotation marks with @@wrapper-code-quote or @wrapper-code-dquote.', array(
      ...
      if ($settings['wrap_abbr']) {
        $output .= '<li>' . $this->t('Wraps abbreviations as “e.g.” to @wrapper-code and adds a thin space (1/6 em) after the dots.</li>', array(
          '@wrapper-code' => '<span class="abbr">e.g.</span>'
        )) . '</li>';
      }
      if ($settings['wrap_numbers']) {
        $output .= '<li>' . $this->t('Wraps large numbers &gt; 1&thinsp;000 with <code>@wrapper-code</code> and inserts thin space for digit grouping.', array(
          '@wrapper-code' => '<span class="number">…</span>'
        )) . '</li>';
      }
      if ($settings['wrap_caps']) {
        $output .= '<li>' . $this->t('Wraps multiple capital letters with <code>@wrapper-code</code>.', array(
      ...
      foreach (UnicodeConversion::map('quotes') as $ascii => $unicode) {
        if ($settings['quotes'][$ascii]) {
          $ascii_to_unicode .= $this->t('Converts <code>@ascii</code> to @unicode', array(
            '@ascii' => $ascii,
            '@unicode' => $unicode,
          ));
