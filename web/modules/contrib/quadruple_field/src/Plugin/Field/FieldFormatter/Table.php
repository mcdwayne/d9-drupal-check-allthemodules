...
class Table extends Base {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    foreach (['first', 'second', 'third', 'fourth'] as $subfield) {
      $element[$subfield . '_column_label'] = [
        '#type' => 'textfield',
        '#title' => $this->t(ucwords($subfield) . ' column label'),
        '#size' => 30,
  ...
  public function settingsSummary() {
    ...
    $summary[] = $this->t('Third column label: %third_column_label', ['%third_column_label' => $settings['third_column_label']]);
    $summary[] = $this->t('Fourth column label: %fourth_column_label', ['%fourth_column_label' => $settings['fourth_column_label']]);
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    $table = ['#type' => 'table'];
    $table['#attributes']['class'][] = 'quadruple-field-table';

    if ($settings['first_column_label'] || $settings['second_column_label'] || 
    	$settings['third_column_label'] || $settings['fourth_column_label']) {
      if ($settings['number_column']) {
        $header[] = $settings['number_column_label'];
      }
      $header[] = $settings['first_column_label'];
      $header[] = $settings['second_column_label'];
      $header[] = $settings['third_column_label'];
      $header[] = $settings['fourth_column_label'];            
      $table['#header'] = $header;
    }
    ...
    foreach ($items as $delta => $item) {
      ...
      foreach (['first', 'second', 'third', 'fourth'] as $subfield) {
        ...
        if ($settings[$subfield]['hidden']) {
          ...
          $row[] = [
            '#theme' => 'quadruple_field_subfield',
            '#settings' => $settings,
