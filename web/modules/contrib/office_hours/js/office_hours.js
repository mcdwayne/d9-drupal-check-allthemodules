  ...
  Drupal.behaviors.office_hours = {
    attach: function (context, settings) {
      ...
      $('.office-hours-copy-link').bind('click', function (e) {
        ...
        var current_day;
        var current_day_week_widget = $(this).closest('tr').find('input')[0].value;
        var current_day_list_widget = $(this).closest('div div').find('select')[0].value;

        current_day = parseInt(current_day_week_widget);
        if(isNaN(current_day_week_widget)) {
          current_day = parseInt(current_day_list_widget);
        }
