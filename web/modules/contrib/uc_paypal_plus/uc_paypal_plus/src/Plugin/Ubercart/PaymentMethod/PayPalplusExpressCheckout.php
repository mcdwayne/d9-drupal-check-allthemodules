...
/**
 * Defines the PayPal Plus Checkout payment method.
 *
 * @UbercartPaymentMethod(
 *   id = "paypalplus_ec",
 *   name = @Translation("PayPal PLUS")
 * )
 */
 //to-do refactor to drop Express from the class name and methods so that it does not create confusion with PayPal Express
class PayPalplusExpressCheckout extends PayPalplusPaymentMethodPluginBase implements ExpressPaymentMethodPluginInterface, OffsitePaymentMethodPluginInterface {
  ...
  public function defaultConfiguration() {
    return parent::defaultConfiguration() + [
      'ecplus_landingpage_style' => 'Billing',
      'ecplus_rqconfirmed_addr' => FALSE,
      'ecplus_review_shipping' => TRUE,
      'ecplus_review_company' => TRUE,
      'ecplus_review_phone' => TRUE,
      'ecplus_review_comment' => TRUE,
      'wppplus_cc_txn_type' => 'Sale',
    ];
  }
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    // PayPal Plus Checkout specific settings.
    $form['ecplus_landingpage_style'] = array(
      '#type' => 'radios',
      '#title' => $this->t('Default PayPal PPP landing page'),
      '#options' => array(
        'Billing' => $this->t('Credit card submission form.'),
        'Login' => $this->t('Account login form.'),
      ),
      '#default_value' => $this->configuration['ecplus_landingpage_style'],
    );
    $form['ecplus_rqconfirmed_addr'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Require PPP Express Checkout users to use a PayPal confirmed shipping address.'),
      '#default_value' => $this->configuration['ecplus_rqconfirmed_addr'],
    );
    $form['ecplus_review_shipping'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Enable the shipping select form on the Review payment page.'),
      '#default_value' => $this->configuration['ecplus_review_shipping'],
    );
    $form['ecplus_review_company'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Enable the company name box on the Review payment page.'),
      '#default_value' => $this->configuration['ecplus_review_company'],
    );
    $form['ecplus_review_phone'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Enable the contact phone number box on the Review payment page.'),
      '#default_value' => $this->configuration['ecplus_review_phone'],
    );
    $form['ecplus_review_comment'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Enable the comment text box on the Review payment page.'),
      '#default_value' => $this->configuration['ecplus_review_comment'],
    );
    $form['wppplus_cc_txn_type'] = array(
      '#type' => 'radios',
      ...
      '#default_value' => $this->configuration['wppplus_cc_txn_type'],
    );
  ...
  public function submitConfigurationForm(array &$form, FormStateInterface $form_state) {
    $this->configuration['ecplus_landingpage_style'] = $form_state->getValue('ecplus_landingpage_style');
    $this->configuration['ecplus_rqconfirmed_addr'] = $form_state->getValue('ecplus_rqconfirmed_addr');
    $this->configuration['ecplus_review_shipping'] = $form_state->getValue('ecplus_review_shipping');
    $this->configuration['ecplus_review_company'] = $form_state->getValue('ecplus_review_company');
    $this->configuration['ecplus_review_phone'] = $form_state->getValue('ecplus_review_phone');
    $this->configuration['ecplus_review_comment'] = $form_state->getValue('ecplus_review_comment');
    $this->configuration['wppplus_cc_txn_type'] = $form_state->getValue('wppplus_cc_txn_type');
  ...
  public function orderView(OrderInterface $order) {
	  //to-do model information for uc_payment_paypal_plus_ipn
    $txn_id = db_query("SELECT txn_id FROM {uc_payment_paypal_plus_ipn} WHERE order_id = :id ORDER BY received ASC", [':id' => $order->id()])->fetchField();
    if (empty($txn_id)) {
      $txn_id = $this->t('Unknown');
    }

    $build['#markup'] = $this->t('Transaction PPP ID:<br />@txn_id', ['@txn_id' => $txn_id]);
    return $build;
  }

  /**
   * Redirect to PayPal Plus Checkout Mark Flow.
   *
  ...
  public function buildRedirectForm(array $form, FormStateInterface $form_state, OrderInterface $order = NULL) {
    ...
    $request = array(
      ...
      'RETURNURL' => Url::fromRoute('uc_paypal_plus.ec_complete', [], ['absolute' => TRUE])->toString(),
      ...
      'PAYMENTACTION' => $this->configuration['wppplus_cc_txn_type'],
      'DESC' => $this->t('Order @order_id at @store', ['@order_id' => $order->id(), '@store' => uc_store_name()]),
      'INVNUM' => $order->id() . '-' . REQUEST_TIME,
      'REQCONFIRMSHIPPING' => $this->configuration['ecplus_rqconfirmed_addr'],
      'ADDROVERRIDE' => 1,
      ...
      'LANDINGPAGE' => $this->configuration['ecplus_landingpage_style'],
    );
    ...
    $response = $this->sendNvpRequesttwo($request);

    if ($response['ACK'] != 'Success') {
      \Drupal::logger('uc_paypal_plus')->error('NVP API request failed with @code: @message', ['@code' => $response['L_ERRORCODE0'], '@message' => $response['L_LONGMESSAGE0']]);
      return $this->t('PayPal reported an error: @code: @message', ['@code' => $response['L_ERRORCODE0'], '@message' => $response['L_LONGMESSAGE0']]);
    }

    $session->set('TOKEN', $response['TOKEN']);
    //to-do use config values for URLS directly. Everything the same way.
    $sandbox = strpos($this->configuration['wppplus_server'], 'sandbox') > 0 ? 'sandbox.' : '';
  ...
  public function orderSubmit(OrderInterface $order) {
    ...
    $response = $this->sendNvpRequesttwo([
    ...
    if ($response['ACK'] != 'Success') {
      \Drupal::logger('uc_paypal_plus')->error('NVP PPP API request failed with @code: @message', ['@code' => $response['L_ERRORCODE0'], '@message' => $response['L_LONGMESSAGE0']]);
      return $this->t('PayPal PPP reported an error: @code: @message', ['@code' => $response['L_ERRORCODE0'], '@message' => $response['L_LONGMESSAGE0']]);
    }
  ...
  public function submitExpressForm(array &$form, FormStateInterface $form_state) {
    ...
    $response = $this->sendNvpRequesttwo([
      'METHOD' => 'SetExpressCheckout',
      'RETURNURL' => Url::fromRoute('uc_paypal_plus.ec_review', [], ['absolute' => TRUE])->toString(),
    ...
    if ($response['ACK'] != 'Success') {
      \Drupal::logger('uc_paypal_plus')->error('NVP PPP API request failed with @code: @message', ['@code' => $response['L_ERRORCODE0'], '@message' => $response['L_LONGMESSAGE0']]);
      drupal_set_message($this->t('PayPal PPP reported an error: @code: @message', ['@code' => $response['L_ERRORCODE0'], '@message' => $response['L_LONGMESSAGE0']]), 'error');
      return;
    }
  ...
  public function getExpressReviewForm(array $form, FormStateInterface $form_state, OrderInterface $order) {
    ...
    if ($this->configuration['ecplus_review_shipping'] && \Drupal::moduleHandler()->moduleExists('uc_quote') && $order->isShippable()) {
    ...
    if ($this->configuration['ecplus_review_company']) {
    ...
    if ($this->configuration['ecplus_review_phone']) {
    ...
    if ($this->configuration['ecplus_review_comment']) {
  ...
  public function submitExpressReviewForm(array $form, FormStateInterface $form_state, OrderInterface $order) {
    ...
    if ($this->configuration['ecplus_review_comment'] && $form_state->getValue('order_comments')) {
   ...
   */
  public function sendNvpRequesttwo($params) {
    $host = $this->configuration['wppplus_server'];
    $params += [
      'USER' => $this->configuration['apiplus']['apiplus_username'],
      'PWD' => $this->configuration['apiplus']['apiplus_password'],
      'SIGNATURE' => $this->configuration['apiplus']['apiplus_signature'],
      'VERSION' => '3.0',
    ];
    ...
    try {
      ...
      \Drupal::logger('uc_paypal_plus')->error('NVP PPP API request failed with HTTP error %error.', ['%error' => $e->getMessage()]);
    }
  }
