 ...
 */
class PayPalplusController extends ControllerBase {
  ...
  public function ipn(Request $request) {
	...
	//to-do are we sure the IPN expects just an empty 200 response??
    return new Response();
  }
  ...
  protected function processIpn($ipn) {
    ...
    if (!isset($ipn['invoice'])) {
      \Drupal::logger('uc_paypal_plus')->error('IPN attempted with invalid order ID.');
	  
	  //to-do shouldn't we return somenthing different here and manage a 403 header response in the final HTTP response?
      return;
    }
    ...
    if (!$order) {
	  ...
	  //to-do shouldn't we return somenthing different here and manage a 403 header response in the final HTTP response?
      \Drupal::logger('uc_paypal_plus')->error('IPN attempted for non-existent order @order_id.', ['@order_id' => $order_id]);
      return;
    }
    ...
    if (!empty($config['wpsplus_debug_ipn'])) {
      \Drupal::logger('uc_paypal_plus')->notice('Receiving IPN at URL for order @order_id. <pre>@debug</pre>', ['@order_id' => $order_id, '@debug' => print_r($ipn, TRUE)]);
    }

    // Express Checkout IPNs may not have the WPS email stored. But if it is,
    // make sure that the right account is being paid.
    if (!empty($config['wpsplus_email']) && Unicode::strtolower($email) != Unicode::strtolower($config['wpsplus_email'])) {
	  //to-do shouldn't we return somenthing different here and manage a 403 header response in the final HTTP response?
      \Drupal::logger('uc_paypal_plus')->error('IPN for a different PayPal PPP account attempted.');
      return;
    }
    ...
    try {
	  ...
	  //to-do shouldn't we return somenthing different here and manage a 403 header response in the final HTTP response?
      \Drupal::logger('uc_paypal_plus')->error('IPN validation failed with HTTP error %error.', ['%error' => $e->getMessage()]);
      return;
    }

    // Check IPN validation response to determine if the IPN was valid..
    if ($response->getBody() != 'VERIFIED') {
      \Drupal::logger('uc_paypal_plus')->error('IPN transaction failed verification.');
	...
	//to-do model uc_payment_paypal_plus_ipn and use Entity Framework for it.
    $duplicate = (bool) db_query_range('SELECT 1 FROM {uc_payment_paypal_plus_ipn} WHERE txn_id = :id AND status <> :status', 0, 1, [':id' => $txn_id, ':status' => 'Pending'])->fetchField();
    if ($duplicate) {
      if ($order->getPaymentMethodId() != 'credit') {
        \Drupal::logger('uc_paypal_plus')->notice('IPN transaction ID has been processed before.');
      }
      return;
    }

    db_insert('uc_payment_paypal_plus_ipn')
      ->fields(array(
        'order_id' => $order_id,
        'txn_id' => $txn_id,
    ...
    switch ($ipn['payment_status']) {
      case 'Canceled_Reversal':
        uc_order_comment_save($order_id, 0, $this->t('PayPal PPP has canceled the reversal and returned @amount @currency to your account.', ['@amount' => uc_currency_format($amount, FALSE), '@currency' => $ipn['mc_currency']]), 'admin');
        break;

      case 'Completed':
        if (abs($amount - $order->getTotal()) > 0.01) {
          \Drupal::logger('uc_paypal_plus')->warning('Payment @txn_id for order @order_id did not equal the order total.', ['@txn_id' => $txn_id, '@order_id' => $order->id(), 'link' => Link::createFromRoute($this->t('view'), 'entity.uc_order.canonical', ['uc_order' => $order->id()])->toString()]);
        }
        $comment = $this->t('PayPal transaction ID: @txn_id', ['@txn_id' => $txn_id]);
        uc_payment_enter($order_id, 'paypalplus_wps', $amount, $order->getOwnerId(), NULL, $comment);
        uc_order_comment_save($order_id, 0, $this->t('PayPal PPP IPN reported a payment of @amount @currency.', ['@amount' => uc_currency_format($amount, FALSE), '@currency' => $ipn['mc_currency']]));
        break;
      ...
      case 'Pending':
        $order->setStatusId('paypalplus_pending')->save();
      ...
      case 'Reversed':
        \Drupal::logger('uc_paypal_plus')->error('PayPal PPP has reversed a payment!');
        uc_order_comment_save($order_id, 0, $this->t('Payment has been reversed by PPP PayPal: @reason', ['@reason' => $this->reversalMessage($ipn['reason_code'])]), 'admin');
        break;
