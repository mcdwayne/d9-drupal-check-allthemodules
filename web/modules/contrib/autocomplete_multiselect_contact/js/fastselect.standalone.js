(function(factory) {
   ...
   $.extend(Fastsearch.prototype, {
      init: function(inputElement, options) {
         options = this.options = $.extend(true, {}, Fastsearch.defaults,
            options);
         this.$input = $(inputElement);
         this.$el = options.wrapSelector instanceof $ ? options.wrapSelector :
            this.$input.closest(options.wrapSelector);
         Fastsearch.pickTo(options, this.$el.data(), ['url',
            'onItemSelect', 'noResultsText', 'inputIdName',
            'apiInputName'
         ]);
         options.url = options.url || this.$el.attr('action');
         this.ens = '.fastsearch' + (++instanceNum);
         this.itemSelector = Fastsearch.selectorFromClass(
            options.itemClass);
      ...
      events: function() {
         ...
         this.$input.on(this.namespaceEvents('keyup focus click'),
            function(e) {
               keyMap[e.keyCode] !== 'enter' && self.handleTyping();
            }).on(this.namespaceEvents('keydown'), function(e) {
            keyMap[e.keyCode] === 'enter' && options.preventSubmit &&
               e.preventDefault();
         ...
         this.$el.on(this.namespaceEvents('click'), this.itemSelector,
            function(e) {
               e.preventDefault();
               self.handleItemSelect($(this));
            });
         options.mouseEvents && this.$el.on(this.namespaceEvents(
            'mouseleave'), this.itemSelector, function(e) {
            $(this).removeClass(options.focusedItemClass);
         }).on(this.namespaceEvents('mouseenter'), this.itemSelector,
            function(e) {
         ...
         if (inputValue.length < this.options.minQueryLength) {
            ...
            this.keyupTimeout = setTimeout(function() {
               ...
               self.getResults(function(data) {
                  self.showResults(self.storeResponse(
      ...
      getResults: function(callback) {
         var self = this,
            options = this.options,
            formValues = this.$el.find('input, textarea, select')
            .serializeArray();
      ...
      generateResults: function(data) {
         ...
         if (data.length === 0) {
            $allResults.html('<p class="' + options.noResultsClass +
               '">' + (typeof options.noResultsText ===
                  'function' ? options.noResultsText.call(
                     this) : options.noResultsText) + '</p>');
         } else {
            if (this.options.responseType === 'html') {
               $allResults.html(data);
            } else {
               this['generate' + (data[0][options.responseFormat
                  .groupItems
               ] ? 'GroupedResults' : 'SimpleResults')](data,
                  $allResults);
            }
         }
      ...
      generateGroupedResults: function(data, $cont) {
         ...
         $.each(data, function(i, groupData) {
            var $group = $('<div class="' + options.groupClass +
               '">').appendTo($cont);
            groupData[format.groupCaption] && $group.append(
               '<h3 class="' + options.groupTitleClass +
            ...
            options.onGroupCreate && options.onGroupCreate
               .call(self, $group, groupData, self);
         });
      },
      generateItem: function(item) {
         var options = this.options,
            ...
            $tag = $('<' + (url ? 'a' : 'span') + '>').html(html)
            .addClass(options.itemClass);
         url && $tag.attr('href', url);
         options.onItemCreate && options.onItemCreate.call(this,
            $tag, item, this);
         return $tag;
      },
      showResults: function($content) {
         ...
         this.$el.removeClass(this.options.loadingClass).addClass(
            this.options.resultsOpenedClass);
         if (this.options.flipOnBottom) {
            this.checkDropdownPosition();
         }
         this.$resultsCont = this.$resultsCont || $('<div>').addClass(
            this.options.resultsContClass).appendTo(this.$el);
         if ($content) {
            this.$resultsCont.html($content);
            this.$resultItems = this.$resultsCont.find(this.itemSelector);
            this.options.onResultsCreate && this.options.onResultsCreate
               .call(this, this.$resultsCont, this.responseData,
                  this);
         }
         ...
         if (this.options.focusFirstItem && this.$resultItems &&
      ...
      checkDropdownPosition: function() {
         ...
         var offset = typeof flipOnBottom === 'boolean' &&
            flipOnBottom ? 400 : flipOnBottom;
         var isFlipped = this.$input.offset().top + offset >
            $document.height();
         this.$el.toggleClass(this.options.resultsFlippedClass,
            isFlipped);
      },
      documentCancelEvents: function(setup, onCancel) {
         ...
         if (setup === 'off' && this.closeEventsSetuped) {
            ...
            $document.on(this.namespaceEvents('click keyup'),
               function(e) {
                  if (keyMap[e.keyCode] === 'escape' || (!$(e
                        .target).is(self.$el) && !$.contains(
                        self.$el.get(0), e.target) && $.contains(
      ...
      navigateItem: function(direction) {
         ...
         if ($currentItem.length === 0) {
            this.$resultItems.eq(direction === 'up' ?
               maxPosition : 0).addClass(this.options.focusedItemClass);
            return;
         }
         var currentPosition = this.$resultItems.index(
               $currentItem),
            nextPosition = direction === 'up' ? currentPosition -
            1 : currentPosition + 1;
         ...
         this.$resultItems.eq(nextPosition).addClass(this.options
            .focusedItemClass);
      },
      ...
      handleItemSelect: function($item) {
         var selectOption = this.options.onItemSelect,
            model = this.itemModels.length ? this.itemModels[
      ...
      fillInput: function(model) {
         ...
         if (options.fillInputId && model.id) {
            if (!this.$inputId) {
               var inputIdName = options.inputIdName || this.$input
                  .attr('name') + '_id';
               this.$inputId = this.$el.find('input[name="' +
                  inputIdName + '"]');
               if (!this.$inputId.length) {
                  this.$inputId = $(
                     '<input type="hidden" name="' +
                     inputIdName + '" />').appendTo(this.$el);
               }
            }
      ...
      destroy: function() {
         ...
         this.$el.off(this.ens).removeClass(this.options.resultsOpenedClass)
   ...
   Fastsearch.defaults = {
      ...
      mouseEvents: !('ontouchstart' in window || navigator.maxTouchPoints >
         0 || navigator.msMaxTouchPoints > 0), // detect if client is touch enabled so plugin can decide if mouse specific events should be set.
   ...
   $.fn.fastsearch = function(options) {
      return this.each(function() {
         if (!$.data(this, 'fastsearch')) {
            $.data(this, 'fastsearch', new Fastsearch(this,
               options));
         }
      });
   };
   ...
   $.extend(Fastselect.prototype, {
      ...
      setupSingleElement: function(onDone) {
         var initialOptions = this.processInitialOptions(),
            toggleBtnText = initialOptions && initialOptions.length ?
            initialOptions[0].text : this.options.placeholder;
         this.$el.addClass(this.options.singleModeClass);
         this.$controls = $('<div>').addClass(this.options.controlsClass);
         this.$toggleBtn = $('<div>').addClass(this.options.toggleButtonClass)
            .text(toggleBtnText).appendTo(this.$el);
         this.$queryInput = $('<input>').attr('placeholder',
            this.options.searchPlaceholder).addClass(this.options
            .queryInputClass).appendTo(this.$controls);
         onDone.call(this);
      },
      setupMultipleElement: function(onDone) {
         ...
         this.$queryInput = $('<input>').addClass(options.queryInputClass)
            .appendTo(this.$controls);
         initialOptions && $.each(initialOptions, function(i,
            option) {
      ...
      processInitialOptions: function() {
         ...
         if (this.hasCustomLoader) {
            ...
            options = $.map(this.$input.find('option:selected').get(),
               function(option) {
                  var $option = $(option);
                  return {
                     text: $option.text(),
      ...
      addChoiceItem: function(optionModel) {
         $('<div data-text="' + optionModel.text +
            '" data-value="' + optionModel.value +
            '" class="' + this.options.choiceItemClass + '">' +
            $('<div>').html(optionModel.text).text() +
            '<button class="' + this.options.choiceRemoveClass +
            '" type="button">×</button>' + '</div>').insertBefore(
            this.$queryInput);
      },
      setupFastsearch: function() {
         var self = this,
            options = this.options,
            fastSearchParams = {};
         pickTo(fastSearchParams, options, ['resultsContClass',
            'resultsOpenedClass', 'resultsFlippedClass',
            'groupClass', 'itemClass', 'focusFirstItem',
            'groupTitleClass', 'loadingClass',
            'noResultsClass', 'noResultsText',
            'focusedItemClass', 'flipOnBottom'
         ]);
         this.fastsearch = new Fastsearch(this.$queryInput.get(0),
            $.extend(fastSearchParams, {
               wrapSelector: this.isMultiple ? this.$el : this
                  .$controls,
               minQueryLength: 0,
               typeTimeout: this.hasCustomLoader ? options
                  .typeTimeout : 0,
               preventSubmit: true,
               fillInputId: false,
               responseFormat: {
               ...
               onItemSelect: function($item, model,
                  ...
                  if (self.isMultiple && maxItems && (
                        self.optionsCollection.getValues()
                        .length > (maxItems - 1))) {
                     ...
                     options.onItemSelect && options.onItemSelect
                        .call(self, $item, model, self,
                           fastsearch);
                  }
               },
               onItemCreate: function($item, model) {
                  model.$item = $item;
                  model.selected && $item.addClass(
                     options.itemSelectedClass);
                  if (self.userOptionAllowed && model.isUserOption) {
                     $item.text(self.options.userOptionPrefix +
                        $item.text()).addClass(self.options
                        .userOptionClass);
                  }
                  options.onItemCreate && options.onItemCreate
                     .call(self, $item, model, self);
               }
            }));
         this.fastsearch.getResults = function() {
            if (self.userOptionAllowed && self.$queryInput.val()
               .length > 1) {
               self.renderOptions();
            }
      ...
      getOptions: function(onDone) {
         ...
         if (this.hasCustomLoader) {
            ...
            !this.optionsCollection.models && this.optionsCollection
               .reset(this.gleanSelectData(this.$input));
            onDone();
         }
      },
      namespaceEvents: function(events) {
         return Fastsearch.prototype.namespaceEvents.call(this,
            events);
      },
      setupEvents: function() {
         var self = this,
            options = this.options;
         if (this.isMultiple) {
            this.$el.on(this.namespaceEvents('click'), function(
               e) {
               $(e.target).is(selectorFromClass(options.controlsClass)) &&
                  self.$queryInput.focus();
            });
            this.$queryInput.on(this.namespaceEvents('keyup'),
               function(e) {
      ...
      adjustQueryInputLayout: function() {
         if (this.isMultiple && this.$queryInput) {
            ...
            this.$queryInput.toggleClass(this.options.queryInputExpandedClass,
               noneSelected);
            if (noneSelected) {
               ...
               this.$fakeInput = this.$fakeInput || $('<span>').addClass(
                  this.options.fakeInputClass);
               this.$fakeInput.text(this.$queryInput.val().replace(
                  /\s/g, '&nbsp;'));
               this.$queryInput.removeAttr('placeholder').css(
                  'width', this.$fakeInput.insertAfter(this.$queryInput)
                  .width() + 20);
      ...
      documentCancelEvents: function(setup) {
         Fastsearch.prototype.documentCancelEvents.call(this,
            setup, this.hide);
      },
      setSelectedOption: function(option) {
         ...
         var selectedModel = this.optionsCollection.findWhere(
            function(model) {
         ...
         if (this.isMultiple) {
            ...
            this.fastsearch && this.fastsearch.$resultItems.removeClass(
               this.options.itemSelectedClass);
            this.$toggleBtn && this.$toggleBtn.text(option.text);
         }
         selectedModel && selectedModel.$item.addClass(this.options
            .itemSelectedClass);
         this.updateDomElements();
      },
      removeSelectedOption: function(option, $choiceItem) {
         var removedModel = this.optionsCollection.removeSelected(
            option);
         ...
         if ($choiceItem) {
            ...
            this.$el.find(selectorFromClass(this.options.choiceItemClass) +
               '[data-value="' + option.value + '"]').remove();
         }
         this.updateDomElements();
         this.writeToInput();
      },
      writeToInput: function() {
         var values = this.optionsCollection.getValues(),
            ...
            formattedValue = this.isMultiple ? (this.hasCustomLoader ?
               values.join(delimiter) : values) : values[0];
      ...
      renderOptions: function(filter) {
         ...
         if (this.optionsCollection.models) {
            data = (filter ? this.optionsCollection.filter(query) :
               this.optionsCollection.models).slice(0);
         } else {
            data = [];
         }
         ...
         this.fastsearch.showResults(this.fastsearch.storeResponse(
            data).generateResults(data));
      },
   ...
   $.extend(OptionsCollection.prototype, {
      ...
      fetch: function(fetchParams, onDone) {
         ...
         if (this.options.loadOnce) {
            this.fetchDeferred = this.fetchDeferred || this.load(
               fetchParams);
      ...
      reset: function(models) {
         this.models = this.options.parseData ? this.options.parseData(
            models) : models;
      ...
      applySelectedValues: function(onDone) {
         this.each(function(option) {
            if (this.options.multipleValues && option.selected) {
               ...
               option.selected = !!this.selectedValues[
                  option.value];
            }
         });
      ...
      load: function(params, onDone) {
         ...
         return $.get(options.url, params, function(data) {
            self.models = options.parseData ? options.parseData(
               data) : data;
      ...
      each: function(iterator) {
         ...
         this.models && $.each(this.models, function(i, option) {
            option.items ? $.each(option.items, function(i,
               nestedOption) {
      ...
      filter: function(query) {
         ...
         function checkItem(item) {
            return self.options.matcher(item.text, query) ? item :
               null;
         }
         ...
         return $.map(this.models, function(item) {
            if (item.items) {
               var filteredItems = $.map(item.items,
                  checkItem);
