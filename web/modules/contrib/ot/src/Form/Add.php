...
class Add extends FormBase
{
  ...
  public function buildForm(array $form, FormStateInterface $form_state)
  {
    ...
    $form['ot_type'] = array(
      ...
      '#default_value'=> 'node_path',
      '#required'=> true
    );

    $form['ot_type_id'] = array(
      '#type'=> 'textfield',
      '#title'=> ($form_state->getValue('ot_type') == 'view') ? 'Views' : t('Node and Path/URL'),
    ...
    $ot_display_title = !empty($form_state->getValue('ot_type_id')) ? $form_state->getValue('ot_type_id') : '';
    $form['ot_display'] = array(
      '#type'=> 'select',
      '#title'=> t('Views dispaly of: '.$ot_display_title),
      '#options'=> array(),
      '#empty_options'=> t('- Select display -'),
      '#required'=> ($form_state->getValue('ot_type') == 'view') ? true : false,
      '#validated' => true,
      '#states'=> array(
        'visible'=> array(
  ...
  public function submitForm(array &$form, FormStateInterface $form_state)
  {
    ...
    try{
      db_insert('override_title')
      ->fields([
        'type'=> $form_state->getValue('ot_type'),
        'uid'=> !empty($user) ? $user : 0,
        'language'=> $language,
        'type_id'=> $form_state->getValue('ot_type_id'),
        'display_id'=> !empty($form_state->getValue('ot_display'))? $form_state->getValue('ot_display') : null,
