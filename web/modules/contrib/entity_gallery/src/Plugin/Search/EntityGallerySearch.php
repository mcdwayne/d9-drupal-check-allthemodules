...
namespace Drupal\entity_gallery\Plugin\Search;
...
/**
 * Handles searching for entity gallery entities using the Search module index.
 *
 * @SearchPlugin(
 *   id = "entity_gallery_search",
 *   title = @Translation("Galleries")
 * )
 */
class EntityGallerySearch extends ConfigurableSearchPluginBase implements AccessibleInterface, SearchIndexingInterface {
  ...
  /**
   * The Renderer service to format the username and entity gallery.
   *
   ...
   *   matched against the {entity_gallery_field_data} table.
   * - operator: (optional) OR or AND, defaults to OR.
   *
   * @var array
   */
  protected $advanced = array(
    'type' => array('column' => 'eg.type'),
    'language' => array('column' => 'i.langcode'),
    'author' => array('column' => 'eg.uid'),
    'term' => array('column' => 'ti.tid', 'join' => array('table' => 'taxonomy_index', 'alias' => 'ti', 'condition' => 'eg.egid = ti.egid')),
  );
  ...
  /**
   * Constructs a \Drupal\entity_gallery\Plugin\Search\EntityGallerySearch object.
   *
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, Connection $database, EntityManagerInterface $entity_manager, ModuleHandlerInterface $module_handler, Config $search_settings, LanguageManagerInterface $language_manager, RendererInterface $renderer, AccountInterface $account = NULL) {
    ...
    $this->addCacheTags(['entity_gallery_list']);
  }
  ...
  public function access($operation = 'view', AccountInterface $account = NULL, $return_as_object = FALSE) {
    $result = AccessResult::allowedIfHasPermission($account, 'access entity galleries');
  ...
  public function isSearchExecutable() {
    // Entity gallery search is executable if we have keywords or an advanced
  ...
  protected function findResults() {
    ...
    $query->join('entity_gallery_field_data', 'eg', 'eg.egid = i.sid AND eg.langcode = i.langcode');
    ...
    $this->addEntityGalleryRankings($query);

    // Run the query.
    $find = $query
      // Add the language code of the indexed item to the result of the query,
      // since the entity gallery will be rendered using the respective
      // language.
  ...
  protected function prepareResults(StatementInterface $found) {
    ...
    $entity_gallery_render = $this->entityManager->getViewBuilder('entity_gallery');
    $keys = $this->keywords;

    foreach ($found as $item) {
      // Render the entity gallery.
      /** @var \Drupal\entity_gallery\EntityGalleryInterface $entity_gallery */
      $entity_gallery = $entity_gallery_storage->load($item->sid)->getTranslation($item->langcode);
      $build = $entity_gallery_render->view($entity_gallery, 'search_result', $item->langcode);

      /** @var \Drupal\entity_gallery\EntityGalleryTypeInterface $type*/
      $type = $this->entityManager->getStorage('entity_gallery_type')->load($entity_gallery->bundle());
      ...
      $rendered .= ' ' . $this->moduleHandler->invoke('comment', 'entity_gallery_update_index', [$entity_gallery]);

      $extra = $this->moduleHandler->invokeAll('entity_gallery_search_result', [$entity_gallery]);

      $language = $this->languageManager->getLanguage($item->langcode);
      $username = array(
        '#theme' => 'username',
        '#account' => $entity_gallery->getOwner(),
      );

      $result = array(
        'link' => $entity_gallery->url('canonical', array('absolute' => TRUE, 'language' => $language)),
        'type' => $type->label(),
        'title' => $entity_gallery->label(),
      ...
      $this->addCacheableDependency($entity_gallery);

      // We have to separately add the entity gallery owner's cache tags because
      ...
      $this->addCacheableDependency($entity_gallery->getOwner());

      if ($type->displaySubmitted()) {
        $result += array(
          'user' => $this->renderer->renderPlain($username),
          'date' => $entity_gallery->getChangedTime(),
        );
      }
   ...
   *
   * This information is being removed from the rendered entity gallery that is
   * used to build the search result snippet. It just doesn't make sense to have
   * it displayed in the snippet.
   *
   * @param array $build
   *   The build array.
   *
  ...
  public function updateIndex() {
    // Interpret the cron limit setting as the maximum number of entity
    // galleries to index per cron run.
    $limit = (int) $this->searchSettings->get('index.cron_limit');

    $query = db_select('entity_gallery', 'eg', array('target' => 'replica'));
    $query->addField('eg', 'egid');
    $query->leftJoin('search_dataset', 'sd', 'sd.sid = eg.egid AND sd.type = :type', array(':type' => $this->getPluginId()));
    ...
    $entity_gallery_storage = $this->entityManager->getStorage('entity_gallery');
    foreach ($entity_gallery_storage->loadMultiple($egids) as $entity_gallery) {
      $this->indexEntityGallery($entity_gallery);
    }
  }
   ...
   */
  protected function indexEntityGallery(EntityGalleryInterface $entity_gallery) {
    $languages = $entity_gallery->getTranslationLanguages();
    $entity_gallery_render = $this->entityManager->getViewBuilder('entity_gallery');

    foreach ($languages as $language) {
      $entity_gallery = $entity_gallery->getTranslation($language->getId());
      // Render the entity gallery.
      $build = $entity_gallery_render->view($entity_gallery, 'search_index', $language->getId());
      ...
      $build['search_title'] = [
        '#prefix' => '<h1>',
        '#plain_text' => $entity_gallery->label(),
        '#suffix' => '</h1>',
        '#weight' => -1000
      ];
      ...
      $extra = $this->moduleHandler->invokeAll('entity_gallery_update_index', [$entity_gallery]);
      foreach ($extra as $t) {
        $text .= $t;
      }

      // Update index, using search index "type" equal to the plugin ID.
      search_index($this->getPluginId(), $entity_gallery->id(), $language->getId(), $text);
    }
  }
  ...
  public function indexClear() {
    // All EntityGallerySearch pages share a common search index "type" equal to
    // the plugin ID.
  ...
  public function indexStatus() {
    $total = $this->database->query('SELECT COUNT(*) FROM {entity_gallery}')->fetchField();
    $remaining = $this->database->query("SELECT COUNT(DISTINCT eg.egid) FROM {entity_gallery} eg LEFT JOIN {search_dataset} sd ON sd.sid = eg.egid AND sd.type = :type WHERE sd.sid IS NULL OR sd.reindex <> 0", array(':type' => $this->getPluginId()))->fetchField();
  ...
  public function searchFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    // Add entity gallery types.
    $types = array_map(array('\Drupal\Component\Utility\Html', 'escape'), entity_gallery_type_get_names());
