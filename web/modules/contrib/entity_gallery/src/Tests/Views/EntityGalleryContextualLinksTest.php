...
/**
 * Tests views contextual links on entity galleries.
 *
 * @group entity_gallery
 */
class EntityGalleryContextualLinksTest extends EntityGalleryTestBase {
   ...
   * @var array
   */
  public static $testViews = array('test_frontpage', 'test_contextual_links');

  /**
   * Tests contextual links.
   */
  public function testEntityGalleryContextualLinks() {
    $this->drupalCreateGalleryType(array('type' => 'page'));
    $this->drupalCreateEntityGallery(array('promote' => 1));
    $this->drupalGet('entity_gallery');

    $user = $this->drupalCreateUser(array('administer entity galleries', 'access contextual links'));
    $this->drupalLogin($user);

    $response = $this->renderContextualLinks(array('entity_gallery:entity_gallery=1:'), 'entity_gallery');
    $this->assertResponse(200);
    $json = Json::decode($response);
    $this->setRawContent($json['entity_gallery:entity_gallery=1:']);

    // @todo Add these back when the functionality for making Views displays
    //   appear in contextual links is working again.
    // $this->assertLinkByHref('gallery/1/contextual-links', 0, 'The contextual link to the view was found.');
  ...
  /**
   * Tests if the entity gallery page works if Contextual Links is disabled.
   *
