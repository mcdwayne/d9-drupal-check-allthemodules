...
/**
 * Tests entity gallery type initial language settings.
 *
 * @group entity_gallery
 */
class EntityGalleryTypeInitialLanguageTest extends EntityGalleryTestBase {
  ...
  protected function setUp() {
    ...
    $web_user = $this->drupalCreateUser(array('bypass entity gallery access', 'administer entity gallery types', 'administer entity_gallery fields', 'administer entity_gallery form display', 'administer entity_gallery display', 'administer languages'));
    $this->drupalLogin($web_user);
  }

  /**
   * Tests the entity gallery type initial language defaults, and modifies them.
   *
   * The default initial language must be the site's default, and the language
   * locked option must be on.
   */
  function testEntityGalleryTypeInitialLanguageDefaults() {
    $this->drupalGet('admin/structure/gallery-types/manage/article');
    ...
    $this->assertNoField('langcode', 'Language is not selectable on gallery add/edit page by default.');
    ...
    $this->drupalGet('gallery/add/article');
    $this->assertField('langcode[0][value]', 'Language is selectable on gallery add/edit page when language not hidden.');
    $this->assertOptionSelected('edit-langcode-0-value', 'hu', 'The initial language is the site default on the entity gallery add page after the site default language is changed.');

    // Tests if the language field can be rearranged on the manage form display
    // tab.
    $this->drupalGet('admin/structure/gallery-types/manage/article/form-display');
    ...
    $this->drupalGet('admin/structure/gallery-types/manage/article/display');
  ...
  function testLanguageFieldVisibility() {
    // Creates an entity gallery to test Language field visibility feature.
    $edit = array(
    ...
    // Loads entity gallery page and check if Language field is hidden by
    // default.
    $this->drupalGet('gallery/' . $entity_gallery->id());
    $language_field = $this->xpath('//div[@id=:id]/div', array(
      ':id' => 'field-language-display',
    ));
    $this->assertTrue(empty($language_field), 'Language field value is not shown by default on entity gallery page.');
    ...
    $this->drupalPostForm('admin/structure/gallery-types/manage/article/display', $edit, t('Save'));
    $this->drupalGet('admin/structure/gallery-types/manage/article/display');
    $this->assertOptionSelected('edit-fields-langcode-type', 'language', 'Language field has been set to visible.');

    // Loads entity gallery page and check if Language field is shown.
    $this->drupalGet('gallery/' . $entity_gallery->id());
    $language_field = $this->xpath('//div[@id=:id]/div', array(
      ':id' => 'field-language-display',
    ));
    $this->assertFalse(empty($language_field), 'Language field value is shown on entity gallery page.');
  }
