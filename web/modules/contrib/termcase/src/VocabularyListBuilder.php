...
class VocabularyListBuilder extends DraggableListBuilder {
  ...
  public function buildHeader() {
    ...
    $header['termcase'] = t('Case Conversion');
  ...
  public function buildRow(EntityInterface $entity) {
    ...
    $vocab_machine_name = $entity->get('vid');
    $config = \Drupal::config('termcase.settings');
    $mode_id = $config->get('termcase_options_' . $vocab_machine_name);
    $termcase_mode = termcase_get_mode($mode_id);

    $row['termcase'] = array('#markup' => $termcase_mode);
