...
/**
 * Provides a 'PromotedViewsBlock' block.
 *
 * Lists nodes from a View.
 * Display the first as a highlighted view mode (defaults to full)
 * and the others as another view mode (defaults to teaser).
 *
 * @Block(
 *  id = "promoted_views_block",
 *  admin_label = @Translation("Promoted Views"),
 * )
 */
class PromotedViewsBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    // @todo add view display
    $views = $this->entityTools->getViews();
    $viewOptions = [];
    foreach ($views as $view) {
      if ($view instanceof View) {
        // @todo filter views that contains node only
        $viewOptions[$view->id()] = $view->id();
      }
    }
  ...
  public function build() {
    ...
    $views = $this->configuration['view_name'];
    $views_display = $this->configuration['view_display'];
    $viewsResult = $this->entityTools->getViewResult($views, $views_display);
    $nodeIds = [];
    foreach ($viewsResult as $row) {
      if ($row instanceof ResultRow) {
        $nodeIds[] = $row->nid;
      }
    }
    $nodes = $this->entityTools->nodeLoadMultiple($nodeIds);
