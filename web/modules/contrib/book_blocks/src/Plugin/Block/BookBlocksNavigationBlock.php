 ...
 *
 * @Block(
 *   id = "book_block_navigation",
 *   admin_label = @Translation("Book Navigation Links"),
 *   category = @Translation("Menus")
 * )
 */
class BookBlocksNavigationBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    if (($node = $this->requestStack->getCurrentRequest()->get('node')) && !empty($node->book['bid'])) {
      ...
      if ($book_id) {
        ...
        $variables['#theme']='book_blocks_navigation';
        $variables['#attached']['library'][]='book_blocks/global-styling';
        $variables['#attributes']=['class' => ['book-blocks-navigation']];
        return $variables;
      }
    }
    return [];
