...
class Utilities {
    ...
    public static function visual_tour_start(array &$form, \Drupal\Core\Form\FormStateInterface $form_state)
    {
    ...
    public static function AddsupportTab(array &$form, \Drupal\Core\Form\FormStateInterface $form_state){
        ...
        $form['markup_support_1'] = array(
            '#markup' => '<h3><b>Feature Request/Contact Us:</b></h3><div>Need any help? We can help you with configuring your Identity Provider. Just send us a query and we will get back to you soon.<br /></div><br>',
        );
	...
	public static function createAuthnRequest($acsUrl, $issuer, $nameid_format, $force_authn = 'false'){
		...
		if( !empty($logout_url) )  {
			...
			if(isset($_SESSION['mo_saml_logout_request'])) {
				...
				if(empty($logout_binding_type) || $logout_binding_type == 'HttpRedirect') {
					...
					//SAMLSPUtilities::postSAMLRequest($logout_url, $samlRequest, $sendRelayState);
				}
			}
		}
	...
	public static function createLogoutRequest($nameId, $sessionIndex = '', $issuer, $destination, $slo_binding_type = 'HttpRedirect'){
		...
		//echo htmlentities($requestXmlStr);// exit();
	...
	public static function validateElement(DOMElement $root)
    {
       ...
       /* Validate referenced xml nodes. */
		...
		// var_dump($objXMLSecDSig); exit;
        foreach ($objXMLSecDSig->getValidatedNodes() as $signedNode) {
			// var_dump($signedNode); exit;
	...
	public static function getEmailandPhone() {
        $Registered_email = \Drupal::config('miniorange_saml.settings')->get('miniorange_saml_customer_admin_email');
        $Registered_phone = \Drupal::config('miniorange_saml.settings')->get('miniorange_saml_customer_admin_phone');
        $array = array(
	...
	{	
        ...
        $url = drupal_get_path('module', 'miniorange_saml'). '/css/miniorange_saml.module.css';
