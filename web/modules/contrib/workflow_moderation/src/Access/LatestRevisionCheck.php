...
namespace Drupal\workflow_moderation\Access;
...
class LatestRevisionCheck implements AccessInterface {
   ...
   *
   * @param \Drupal\workflow_moderation\ModerationInformationInterface $moderation_information
   *   The moderation information service.
   */
  ...
  protected function loadEntity(Route $route, RouteMatchInterface $route_match) {
    $entity_type = $route->getOption('_workflow_moderation_entity_type');
    if ($entity = entity_load($entity_type, $route_match->getParameter($entity_type))) {
