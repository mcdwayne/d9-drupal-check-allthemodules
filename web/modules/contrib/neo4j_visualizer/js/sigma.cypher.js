;(function (undefined) {
    ...
    sigma.neo4j.cypher_parse = function (result) {
        ...
        result.results[0].data.forEach(function (data) {
            var id, label, size, color;
            // iteration on graph for all node
            data.graph.nodes.forEach(function (node) {
                if (node.labels.indexOf("entity:node") != -1) {
                    id = node.id;
                    label = node.properties.title;
                    size = 8;
                    color = "#3278bc";
                } else if (node.labels.indexOf("entity:taxonomy_term") != -1) {
                    id = node.id;
                    label = node.properties.name;
                    size = 8;
                    color = "#009688";
                } else if (node.labels.indexOf("entity:relation") != -1) {
                    id = node.id;
