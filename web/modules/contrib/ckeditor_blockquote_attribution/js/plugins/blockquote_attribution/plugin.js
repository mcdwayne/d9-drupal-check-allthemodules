...
( function() {
    ...
    var commandObject = {
        exec: function( editor ) {
            var state = editor.getCommand( 'blockquote_attribution' ).state,
            ...
            if ( state == CKEDITOR.TRISTATE_OFF ) {
                ...
                var figBlock = editor.document.createElement('figure');
                figBlock.setAttribute('class', 'quote');
                figBlock.insertBefore(paragraphs[ 0 ] );
                figBlock.append(bqBlock);
                var figCaption = editor.document.createElement('figcaption' );
                figCaption.setText("Quote source description, ");
                figBlock.append(figCaption);
                var citeTag = editor.document.createElement('cite');
                citeTag.setText('Source Work Title');
                figCaption.append(citeTag);
//                bqBlock.insertBefore( paragraphs[ 0 ] );
        ...
        allowedContent: 'figure(quote) blockquote',
        requiredContent: 'blockquote'
    };

    CKEDITOR.plugins.add( 'blockquote_attribution', {
        // jscs:disable maximumLineLength
        // jscs:enable maximumLineLength
        icons: 'blockquote_attribution', // %REMOVE_LINE_CORE%
        hidpi: false, // %REMOVE_LINE_CORE%
        init: function( editor ) {
            if ( editor.blockless )
                return;

            editor.addCommand( 'blockquote_attribution', commandObject );

            editor.ui.addButton && editor.ui.addButton( 'blockquote_attribution', {
                label: Drupal.t("Blockquote with Attribution"),
                command: 'blockquote_attribution',
                toolbar: 'blocks,10'
            } );
        }
    } );
