...
class CodelistsController {
  ...
  private $nids = null;
  private $node = null;
  private $childrenids = null;
  private $tid = null;
  
  public function showCodelist($namespace, $code) {
    $this->namespace = $namespace;
    $this->buildFormatandCode($_GET['format'], $code);
    $this->buildLang($_GET['lang']);
    $this->checkCodelistExistens();
  ...
  private function checkCodelistExistens() {
    //Check Namespace
    ...
    $this->nids = \Drupal::entityQuery('node')
      ...
      ->condition('field_namespace', $this->tid)
  ...
  public function changeResponse() {
    switch ($this->format) {
      ...
      case 'xml':
        $response = new CodelistsXML();
        return $response->getResponse($this->nids, $this->namespace, $this->tid, $this->lang);
      case 'exml':
        $response = new CodelistsXML();
        return $response->getResponse($this->nids, $this->namespace, $this->tid, $this->lang, true);
      case 'json':
        $response = new CodelistsJSON();
        return $response->getResponse($this->nids, $this->namespace, $this->tid, $this->lang);
    }
  }
}
