 ...
 *
 * @QueueWorker(
 *   id = "advertising_product_amazon_provider_update",
 *   title = @Translation("Advertising product update for amazon updates"),
 *   cron = {"time" = 60}
 * )
 */
class AdvertisingProductAmazonUpdate extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    ...
    try {
      $result = $provider->updateProduct($product_id, $entity_id);
    }
    catch (Exception $e) {
      // we got some sort of lookup error and need to keep trying.
