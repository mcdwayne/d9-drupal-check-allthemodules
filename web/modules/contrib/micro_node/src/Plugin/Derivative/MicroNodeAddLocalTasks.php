...
//  }
  ...
  public function getDerivativeDefinitions($base_plugin_definition) {
    ...
    $node_types_tab = $this->configFactory->get('micro_node.settings')->get('node_types_tab');

    if (empty($node_types_enabled) || empty($node_types_tab)) {
    ...
    foreach ($node_types as $id => $node_type) {
     if (in_array($id, $node_types_enabled) && in_array($id, $node_types_tab) && in_array($id, $site_types_tab)) {
