...
Drupal.ProgressBar = function (id, updateCallback, method, errorCallback) {
  ...
  if (Drupal.settings.nyan.enabled == 1) {
    $('body').append('<audio loop="loop" autoplay="true" id="nyanaudio">' +
                       '<source src="' + Drupal.settings.nyan.mp3 + '" type="audio/mpeg">' +
                       '<source src="' + Drupal.settings.nyan.ogg + '" type="audio/ogg">' +
                     '</audio>');

    // Add controls if needed.
    if (Drupal.settings.nyan.controls_visible == 1) {
      document.getElementById('nyanaudio').controls = 'true';
    }

    // Change the volume, as it sounds a bit messy at full volume.
    document.getElementById('nyanaudio').volume = Drupal.settings.nyan.volume_default;
  }
};
