...
class FormConfigurationTest extends AkismetTestBase {
  ...
  function testFormFieldsConfiguration() {
    ...
    $edit = [
      'mode' => FormInterface::AKISMET_MODE_ANALYSIS,
      'checks[spam]' => TRUE,
    ...
    $this->drupalPostForm('admin/config/content/akismet/form/akismet_test_post_form/delete', array(), t('Remove Akismet Protection'));
  ...
  function testFormAdministration() {
    ...
    $this->assertFalse($this->loadAkismetConfiguredForm('user_register_form'), t('Form configuration does not exist.'));
    ...
    $this->drupalPostForm(NULL, array(), t('Create Protected Akismet Form'));

    // Verify that user registration form was protected.
    $this->assertText(t('The form protection has been added.'));
    $this->assertText($form_info['user_register_form']['title']);
    $this->assertTrue($this->loadAkismetConfiguredForm('user_register_form'), t('Form configuration exists.'));
  ...
  function testInvalidForms() {
    ...
    foreach ($forms as $form_id) {
      ...
      $this->drupalPostForm('admin/config/content/akismet/form/' . $form_id . '/delete', array(), t('Remove Akismet Protection'));
      $this->assertNoLinkByHref('admin/config/content/akismet/form/' . $form_id . '/delete');
    }
    // Confirm deletion.
    $configured = \Drupal::entityManager()->getStorage('akismet_form')->loadMultiple();
  ...
  function testFormAlter() {
    ...
    $this->drupalLogin($this->adminUser);
    $this->setProtectionUI('user_pass', FormInterface::AKISMET_MODE_CAPTCHA);
    $this->drupalLogout();
  ...
  protected function loadAkismetConfiguredForm($id) {
    return \Drupal::entityManager()->getStorage('akismet_form')->load($id);
  }
}
