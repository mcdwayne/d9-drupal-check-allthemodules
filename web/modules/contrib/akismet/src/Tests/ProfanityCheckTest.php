 ...
 * @group akismet
 */
class ProfanityCheckTest extends AkismetTestBase {
  ...
  function testProfanity() {
    // Protect Akismet test form but do not enable profanity checking.
    $this->drupalLogin($this->adminUser);
    $edit_config = [
      'checks[spam]' => TRUE,
      'checks[profanity]' => FALSE,
    ];
    $this->setProtectionUI('akismet_test_post_form', FormInterface::AKISMET_MODE_ANALYSIS, NULL, $edit_config);
    $this->drupalLogout();
  ...
  function testProfanitySpam() {
    ...
    $this->assertAkismetData('akismet_test_post', $mid, 'contentId', $contentId);
