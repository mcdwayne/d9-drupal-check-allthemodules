...
class DataTest extends AkismetTestBase {
  ...
  function testAnalysis() {
    ...
    $data = $this->getServerRecord();
    $this->assertFalse($data, t('Administrative form submission was not validated by Akismet.'));
  }
  ...
  function testHoneypot() {
    ...
    $this->assertFieldByName('akismet[homepage]', '', t('Hidden honeypot field found.'));

    // Verify that a honeypot value is sent to akismet.checkContent.
    $edit = [
      'title' => 'unsure',
      'body' => 'unsure',
      'akismet[homepage]' => 'HONEYPOT-VALUE',
    ];
    $this->drupalPostForm(NULL, $edit, t('Save'));
    $this->assertCaptchaField();
    $data = $this->getServerRecord();
    $this->assertSame('honeypot', $data['honeypot'], $edit['akismet[homepage]']);
    ...
    $this->drupalLogin($this->adminUser);
    $this->setProtectionUI('akismet_test_post_form', FormInterface::AKISMET_MODE_CAPTCHA);
    $this->drupalLogout();
    $this->resetServerRecords();
    $this->resetAll();
    ...
    // Verify that a honeypot value is sent to akismet.checkContent.
    // postCorrectCaptcha() cannot be used for akismet_test_post_form, since the form
  ...
  function testPostIdMapping() {
    ...
    $this->assertSame('entity', $data->entity, 'akismet_test_post');
    $this->assertSame('id', $data->id, $mid);
    $this->assertSame('form_id', $data->form_id, 'akismet_test_post_form');
    $stored = ResponseDataStorage::loadByEntity('akismet_test_post', $mid);
    $this->assertEqual(count($stored), 1, t('Data was stored in {akismet}.'));
    ...
    $this->assertEqual(count($stored), 1, t('Stored data in {akismet} was updated.'));
  }
