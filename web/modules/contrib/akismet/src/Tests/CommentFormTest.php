...
class CommentFormTest extends AkismetTestBase {
  ...
  protected function verifyCommentAnalysis($form_id, NodeInterface $node) {
    ...
    $this->assertAkismetData('comment', reset($cids), 'contentId', $contentId);
  ...
  function verifyCaptcha($form_id, NodeInterface $node) {
    // Enable Akismet CAPTCHA protection for comments.
    $this->drupalLogin($this->adminUser);
    $this->setProtectionUI($form_id, FormInterface::AKISMET_MODE_CAPTCHA);
    $this->drupalLogout();
    ...
    $this->assertAkismetData('comment', reset($cids), 'captchaId', $captchaId);
