...
class ServerController extends ControllerBase {
  ...
  const KEY_SITES = 'akismet_test_server_sites';
  ...
  /**
   * REST callback for akismet.checkContent to perform textual analysis.
   *
  ...
  /**
   * REST callback for akismet.sendFeedback to send feedback for a moderated post.
   *
   * @return \Symfony\Component\HttpFoundation\Response
   *   A valid HTTP response.
   */
  ...
  /**
   * API callback for akismet.checkContent to perform textual analysis.
   *
  ...
  /**
   * API callback for akismet.getImageCaptcha to fetch a CAPTCHA image.
   */
  protected function getCaptcha($data) {
    ...
    $response['url'] = $base_url . '/' . drupal_get_path('module', 'akismet') . '/images/powered-by-akismet-2.gif?captchaId=' . $captchaId;
  ...
  /**
   * API callback for akismet.checkCaptcha to validate a CAPTCHA response.
   */
