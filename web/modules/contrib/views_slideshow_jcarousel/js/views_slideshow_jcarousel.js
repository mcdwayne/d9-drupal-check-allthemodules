...
Drupal.behaviors.viewsSlideshowJcarouselPager = { 
  attach: function (context) {
    ...
    $('.views_slideshow_jcarousel_pager:not(.views-slideshow-jcarousel-pager-processed)', context).addClass('views-slideshow-jcarousel-pager-processed').each(function() {
      ...
      $(this).jcarousel({
        vertical: parseInt(drupalSettings.viewsSlideshowJCarouselPager[uniqueID][location].orientation),
        scroll: parseInt(drupalSettings.viewsSlideshowJCarouselPager[uniqueID][location].scroll),
        visible: parseInt(drupalSettings.viewsSlideshowJCarouselPager[uniqueID][location].visible),
        wrap: drupalSettings.viewsSlideshowJCarouselPager[uniqueID][location].wrap,
        animation: (isNaN(drupalSettings.viewsSlideshowJCarouselPager[uniqueID][location].animation)) ? drupalSettings.viewsSlideshowJCarouselPager[uniqueID][location].animation : parseInt(drupalSettings.viewsSlideshowJCarouselPager[uniqueID][location].animation),
        initCallback: function(carousel) {
          drupalSettings.viewsSlideshowJCarouselPager[uniqueID][location]['carouselObj'] = carousel;
        }
      });
      ...
      $(this).find('.views_slideshow_jcarousel_pager_item').each(function(index, pagerItem) {
        $(pagerItem).click(function() {
          Drupal.viewsSlideshow.action({ "action": 'goToSlide', "slideshowID": uniqueID, "slideNum": index });
			...
			if (drupalSettings.viewsSlideshowJCarouselPager[uniqueID][location].updateOnPrevNext) {
...
Drupal.viewsSlideshowJcarouselPager.transitionBegin = function (options) {
  for(pagerLocation in drupalSettings.viewsSlideshowPager[options.slideshowID]) {
    if (drupalSettings.viewsSlideshowJCarouselPager[options.slideshowID][pagerLocation].moveOnChange) {
      drupalSettings.viewsSlideshowJCarouselPager[options.slideshowID][pagerLocation]['carouselObj'].scroll(options.slideNum);
    }
...
Drupal.viewsSlideshowJcarouselPager.goToSlide = function (options) {
  for(pagerLocation in drupalSettings.viewsSlideshowPager[options.slideshowID]) {
    if (drupalSettings.viewsSlideshowJCarouselPager[options.slideshowID][pagerLocation].moveOnChange) {
      drupalSettings.viewsSlideshowJCarouselPager[options.slideshowID][pagerLocation].scroll(options.slideNum);
    }
        // Remove active class from pagers
    $('[id^="views_slideshow_jcarousel_pager_item_' + pagerLocation + '_' + options.slideshowID + '"]').removeClass('active');
    //console.log("views_slideshow_jcarousel_pager_item_"+ pagerLocation + '_' + options.slideshowID + '_' + options.slideNum);
...
Drupal.viewsSlideshowJcarouselPager.previousSlide = function (options) {
  for(pagerLocation in drupalSettings.viewsSlideshowPager[options.slideshowID]) {
    ...
    if (drupalSettings.viewsSlideshowJCarouselPager[options.slideshowID][pagerLocation].moveOnChange) {
      drupalSettings.viewsSlideshowJCarouselPager[options.slideshowID][pagerLocation].scroll(pagerNum);
    }
  }
};
