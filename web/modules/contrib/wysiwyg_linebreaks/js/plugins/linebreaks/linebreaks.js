...
CKEDITOR.plugins.add('linebreaks', {
  init: function (editor) {
    // On init, grab the contents of the editor, and convert linebreaks.
    var clean = linebreaks_attach(editor.getData());
    editor.setData(clean);

    // If configured to force linebreaks, attach our 'detach' method to the
    // editor's 'destroy' event.
    if (editor.config.linebreaks_method == 'force') {
      editor.on('destroy', function(e) {
        var clean = linebreaks_detach(e.editor.getData());

        // Since the editor is already gone, we need to set the content of the
        // textarea driving the editor directly. We have to use a timeout to
        // ensure our content is written to the textarea last.
        var textarea = $('textarea[data-drupal-selector="' + e.editor.name + '"]');
        setTimeout(function() {
          textarea.val(clean);
        }, 0);
      });
...
// Prepare the content by converting line breaks to paragraph/br tags.
function linebreaks_attach(content) {
...
// Prepare the content by paragraph/br tags to line breaks.
function linebreaks_detach(content) {
