...
namespace Drupal\vuetable_view_rest\Plugin\views\display;
 ...
 * @ViewsDisplay(
 *   id = "vue_table_rest",
 *   title = @Translation("Vue Table Data REST"),
 *   help = @Translation("Create a REST export for a Vue.js table."),
 *   uses_route = TRUE,
 *   admin = @Translation("Vue Table REST"),
 *   returns_response = TRUE
 * )
 */
class VueTableREST extends PathPluginBase implements ResponseDisplayPluginInterface {
  ...
  /**
   * Constructs a VueTableREST object.
   *
  ...
  public function render() {
    ...
    //Add the pagination information into the markup
    $build['#markup'] = $this->addPaginationInfo($build['#markup']);
  ...
  /**
   * Add pagination information into the output data
   * @param original $data
   * @return output string with pagination information|string
   */
  private function addPaginationInfo($data) {
    $base_path = \Drupal::request()->getSchemeAndHttpHost();
    $query_string = rawurldecode(\Drupal::request()->getQueryString());
    ...
    if (isset($query_array['sort_by']) && isset($query_array['sort_order'])) {
      $query_string = '&sort_by=' . $query_array['sort_by'] . '&sort_order=' . $query_array['sort_order'];
    }
    else {
      $query_string = '';
    }
    ...
    $per_page = $pager->getItemsPerPage();
    $last_page = (int) ceil($total / $per_page);
    $current_page = $pager->getCurrentPage() + 1;
    $next_page_url = ($last_page == $current_page) ? 'null' : '"' . str_replace('/', '\/', $base_path . '/' . $this->view->getPath() . '?page=' . ($current_page) . $query_string) . '"';
    $prev_page_url = (1 == $current_page) ? 'null' : '"' . $base_path . '/' . $this->view->getPath() . '?page=' . ($current_page - 2) . $query_string .'"';
