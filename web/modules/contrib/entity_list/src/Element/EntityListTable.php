...
/**
 * Class EntityListTable.
 *
 * Copied from FieldUiTable.
 *
 * @package Drupal\entity_list\Element
 *
 * @FormElement("entity_list_table")
 */
class EntityListTable extends Table {
  ...
  public static function tablePreRender(array $elements) {
    ...
    while ($list) {
      foreach ($list as $name) {
        $row = &$elements[$name];
        $parent = $row['parent_wrapper']['parent']['#value'] ?? NULL;
        ...
        if (empty($parent) || isset($parents[$parent])) {
          ...
          if ($depth = count($parents[$name])) {
            ...
            $row[$cell]['#prefix'] = \Drupal::service('renderer')
              ->render($indentation);
          }
    ...
    $elements['#attached']['drupalSettings']['entityListTableRowsData'] = $js_settings;
