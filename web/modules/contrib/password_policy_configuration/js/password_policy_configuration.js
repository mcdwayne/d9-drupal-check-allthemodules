(function ($, Drupal, drupalSettings) {
    ...
    attach: function attach(context, settings) {
      ...
      if ($passwordInput.length) {
        ...
        var passwordCheck = function passwordCheck() {
          if (settings.password.showStrengthIndicator) {
            var result = Drupal.evaluatePasswordStrength($passwordInput.val(), settings.password, settings);
  ...
  Drupal.evaluatePasswordStrength = function (password, translate, settings) {
    ...
    var allFoundCharacters = password.match(/[@#$%!^&*()_+\-=\[\]{};':"\\|,.<>\/?]/g);
    if (allFoundCharacters)
      punctuationCount = allFoundCharacters.length;

    var $usernameBox = $('input.username');
    var username = $usernameBox.length > 0 ? $usernameBox.val() : translate.username;

    if (password.length < settings.passlength) {
      msg.push(translate.tooShort);
      strength -= (8 - password.length) * 5 + 30;
    }

    if (lowerCaseCount < settings.lowercase) {
      msg.push(translate.addLowerCase);
      weaknesses++;
    }
    if (upperCaseCount < settings.uppercase) {
      msg.push(translate.addUpperCase);
      weaknesses++;
    }
    if (numbersCount < settings.numeric_length) {
      msg.push(translate.addNumbers);
      weaknesses++;
    }
    if (punctuationCount <  settings.punctuation_length) {
