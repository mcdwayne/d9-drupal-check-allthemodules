...
use Drupal\better_exposed_filters\Plugin\BetterExposedFiltersFilterWidgetManager;
use Drupal\better_exposed_filters\Plugin\BetterExposedFiltersPagerWidgetManager;
use Drupal\better_exposed_filters\Plugin\BetterExposedFiltersSortWidgetManager;
...
class BetterExposedFilters extends InputRequired {
  ...
  /**
   * @var \Drupal\better_exposed_filters\Plugin\BetterExposedFiltersFilterWidgetManager
   */
  public $filterWidgetManager;

  /**
   * @var \Drupal\better_exposed_filters\Plugin\BetterExposedFiltersPagerWidgetManager
   */
  public $pagerWidgetManager;

  /**
   * @var \Drupal\better_exposed_filters\Plugin\BetterExposedFiltersSortWidgetManager
   */
   ...
   * {@inheritdoc}
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, BetterExposedFiltersFilterWidgetManager $betterExposedFiltersFilterWidget, BetterExposedFiltersPagerWidgetManager $betterExposedFiltersPagerWidget, BetterExposedFiltersSortWidgetManager $betterExposedFiltersSortWidget) {
    parent::__construct($configuration, $plugin_id, $plugin_definition);
    $this->filterWidgetManager = $betterExposedFiltersFilterWidget;
    $this->pagerWidgetManager = $betterExposedFiltersPagerWidget;
    $this->sortWidgetManager = $betterExposedFiltersSortWidget;
  }
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    if ($exposed) {
      // Array for possible format options.
      $display_options = [];

      // Get the available sort widgets.
      foreach (array_keys($this->sortWidgetManager->getDefinitions()) as $widget_id) {
        /** @var \Drupal\better_exposed_filters\Plugin\BetterExposedFiltersSortWidgetInterface $widget */
        $widget = $this->sortWidgetManager->createInstance($widget_id);
        if ($widget->isApplicable()) {
          $display_options[$widget_id] = $widget->label();
        }
      }

      $bef_options['sort']['bef_format'] = [
        '#type' => 'select',
      ...
      $bef_options['sort']['advanced'] = [
        '#type' => 'details',
        '#title' => $this->t('Advanced sort options'),
      ];
      $bef_options['sort']['advanced']['collapsible'] = [
        '#type' => 'checkbox',
      ...
      $bef_options['sort']['advanced']['collapsible_label'] = [
        '#type' => 'textfield',
        ...
        '#states' => [
          'visible' => [
            ':input[name="exposed_form_options[bef][sort][advanced][collapsible]"]' => ['checked' => TRUE],
          ],
          'required' => [
            ':input[name="exposed_form_options[bef][sort][advanced][collapsible]"]' => ['checked' => TRUE],
          ],
      ...
      $bef_options['sort']['advanced']['combine'] = [
        '#type' => 'checkbox',
      ...
      $bef_options['sort']['advanced']['combine_rewrite'] = [
        '#type' => 'textarea',
...
Title Desc|Z -> A</pre> Leave the replacement text blank to remove an option altogether. The order the options appear will be changed to match the order of options in this field.'),
      ...
      $bef_options['sort']['advanced']['reset'] = [
        '#type' => 'checkbox',
      ...
      $bef_options['sort']['advanced']['reset_label'] = [
        '#type' => 'textfield',
        ...
        '#states' => [
          'visible' => [
            ':input[name="exposed_form_options[bef][sort][advanced][reset]"]' => ['checked' => TRUE],
          ],
          'required' => [
            ':input[name="exposed_form_options[bef][sort][advanced][reset]"]' => ['checked' => TRUE],
          ],
      ...
      $bef_options['sort']['advanced']['is_secondary'] = [
        '#type' => 'checkbox',
    ...
    $pager_settings = $this->view->displayHandlers->getConfiguration();
    /*
     * Add options for exposed pager.
     */
    if (isset($pager_settings['default']['display_options']['pager']) && $pager_settings['default']['display_options']['pager']['options']['expose']['items_per_page']) {

      $display_options = [];
      foreach (array_keys($this->pagerWidgetManager->getDefinitions()) as $widget_id) {
        /** @var \Drupal\better_exposed_filters\Plugin\BetterExposedFiltersPagerWidgetInterface $widget */
        $widget = $this->pagerWidgetManager->createInstance($widget_id);
        if ($widget->isApplicable()) {
          $display_options[$widget_id] = $widget->label();
        }
      }

      $bef_options['pager']['bef_format'] = [
        '#type' => 'select',
      ...
      $bef_options['pager']['is_secondary'] = [
        '#type' => 'checkbox',
        '#title' => $this->t('This is a secondary option'),
    ...
    foreach ($this->view->display_handler->getHandlers('filter') as $label => $filter) {
      ...
      foreach (array_keys($this->filterWidgetManager->getDefinitions()) as $widget_id) {
        /** @var \Drupal\better_exposed_filters\Plugin\BetterExposedFiltersFilterWidgetInterface $widget */
        $widget = $this->filterWidgetManager->createInstance($widget_id);
        if ($widget->isApplicable($filter, $this->displayHandler->handlers['filter'][$label]->options)) {
      ...
      foreach ($available_widgets as $widget) {
        $widget->configurationFormAlter($bef_options, $label, $filter, $existing);
      }
  ...
  public function exposedFormAlter(&$form, FormStateInterface $form_state) {
    ...
    if (isset($settings['sort']) && !empty($form['sort_by'])) {
      $sort_widget_id = $settings['sort']['bef_format'];

      /** @var \Drupal\better_exposed_filters\Plugin\BetterExposedFiltersSortWidgetInterface $widget */
      $widget = $this->sortWidgetManager->createInstance($sort_widget_id);
      $show_apply = TRUE;
      ...
      if ($settings['sort']['advanced']['combine'] && !empty($form['sort_order'])) {
        ...
        // Use the sort widget to format the sort_bef_combine form element.
        $widget->exposedFormAlter($form, $form_state, 'sort_bef_combine');
        ...
        // Use the sort widget to format the sort_by form element.
        $widget->exposedFormAlter($form, $form_state, 'sort_by');
        if (!empty($form['sort_order'])) {
          // Use the sort widget to format the sort_order form element if present.
          $widget->exposedFormAlter($form, $form_state, 'sort_order');
        }
      ...
      if ($allow_secondary && $settings['sort']['advanced']['is_secondary']) {
        foreach (['sort_bef_combine', 'sort_by', 'sort_order'] as $elem) {
          if (!empty($form[$elem])) {
    ...
    if (isset($settings['pager'])) {
      $pager_widget_id = $settings['pager']['bef_format'];

      /** @var \Drupal\better_exposed_filters\Plugin\BetterExposedFiltersPagerWidgetInterface $widget */
      $widget = $this->pagerWidgetManager->createInstance($pager_widget_id);
      $widget->exposedFormAlter($form, $form_state);
      $show_apply = TRUE;

      // Check if this is a secondary form element.
      if ($allow_secondary && $settings['pager']['is_secondary']) {
        foreach (['items_per_page', 'offset'] as $elem) {
          if (!empty($form[$elem])) {
            $secondary[$elem] = $form[$elem];
            unset($form[$elem]);
          }
    ...
    foreach ($settings as $label => $options) {
      ...
      $widget_id = $this->options['bef'][$label]['bef_format'];
      ...
      $widget->exposedFormAlter($form, $form_state, $field_id, $show_apply);
