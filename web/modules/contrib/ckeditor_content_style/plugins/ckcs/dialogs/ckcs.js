...
(function ($) {
  ...
  (function () {
    ...
    function findDialog(editor, startupPage) {
      ...
      /**
       * Search in char range.
       * @param none.
       * @returns character range.
       */
      ...
      var ckcs_text = drupalSettings.ckcs.suspected;
      if (ckcs_text === null || ckcs_text === undefined) {
        ckcs_count = 0;
      }
      else {
        var ckcs_count = Object.keys(ckcs_text).length;
      }
      var total_sus_word = [];
      var chek_value_chkd = [];
      var i;
      ...
      var k;
      return {
        title: Drupal.t('Content Style Guide'),
        ...
        contents: [{
          id: 'replace',
          label: Drupal.t('Replace Suspected Words'),
          accessKey: 'M',
          elements: [
            {
              type: 'text',
              id: 'txtFindReplace',
              label: Drupal.t('Suspected'),
              isChanged: false,
            ...
            },
            {
              ...
              onClick: function () {
                ...
                if (!finder.replace(
                  ...
                  alert(Drupal.t('The specified text was not found move to another word.'));
                }
              }
            },
            {
              ...
              onClick: function () {
                var dialog = this.getDialog();
                var check_word_ext = false;
                if (ckcs_count > 0) {
                  for (i = 0; i <= ckcs_count - 1; i++) {

                    var txttofind = ckcs_text[i].text.trim();
                    var txttorplc = ckcs_text[i].entity;
                    var txtsug = ckcs_text[i].sugestion;
                    if (chek_value_chkd.indexOf(txttofind) !== -1) {
                      continue;
                    }

                    chek_value_chkd.push(txttofind);
                    if (finder.replace(
                        dialog,
                        txttofind,
                        dialog.getValueOf('replace', 'txtReplace'),
                      ...
                      dialog.setValueOf('replace', 'txtFindReplace', txttofind);
                      dialog.setValueOf('replace', 'txtReplace', txttorplc);
                      dialog.setValueOf('replace', 'txtSug', txtsug);
                      check_word_ext = true;
                      break;
                    }
                  }
                }
                if (check_word_ext === false) {
                  alert(Drupal.t('No more suspected words found.'));
            ...
            },
            {
              type: 'textarea',
              id: 'alltxtsup',
              label: Drupal.t('All Suspected Words'),
              labelLayout: 'horizontal',
              readonly: true,
              accessKey: 'alts'
            },
              ...
              children: [{
                ...
                children: [
                  ...
                  },
                  {
                    ...
                    isChanged: false,
                    'default': true,
                    label: Drupal.t('Match Whole Word')
                  },
                  {
                    type: 'checkbox',
                    id: 'txtReplaceCyclic',
        ...
        onShow: function () {
          ...
          if (ckcs_count > 0) {
            for (k = 0; k <= ckcs_count - 1; k++) {
              var txttofind = ckcs_text[k].text.trim();
              ...
              if (finder.find(
                ...
                total_sus_word.push(txttofind);
              }
            }
            dialog.setValueOf('replace', 'alltxtsup', total_sus_word);

            for (j = 0; j <= ckcs_count - 1; j++) {
              var txttofind = ckcs_text[j].text.trim();
              var txttorplc = ckcs_text[j].entity;
              var txtsug = ckcs_text[j].sugestion;
              chek_value_chkd.push(txttofind);
          ...
          if (total_sus_word.length < 1) {
            alert(Drupal.t('None of the suspected words found.'));
            CKEDITOR.dialog.getCurrent().hide();
          }
        ...
        onFocus: function () {
          return this.getContentElement('ckcs', 'txtFindFind');
        }
      };
    }

    CKEDITOR.dialog.add('ckcs', function (editor) {
      return findDialog(editor, 'ckcs');
    });
