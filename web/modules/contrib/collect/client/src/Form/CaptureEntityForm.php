...
/**
 * @file
 * Contains \Drupal\collect_client\Form\CaptureEntityForm.
 */
...
class CaptureEntityForm extends FormBase {
  ...
  public function getFormId() {
    return 'collect_client_entity_capture';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $message_context = array('%entity_type' => $this->entityManager->getStorage($entity_type)->getEntityType()->getLabel());

    // Capture an entity.
    if ($form_state->getValue('operation') == 'single') {
      ...
      $form_state->setRedirect('collect_client.capture_entities');
    }
    ...
    if ($form_state->getValue('operation') == 'multiple') {
      ...
      if (!empty($entities)) {
        ...
        foreach ($entities as $entity) {
          $operations[] = array(
            'collect_client_capture_entity_batch',
            array($entity)
          );
        }
        $batch = array(
