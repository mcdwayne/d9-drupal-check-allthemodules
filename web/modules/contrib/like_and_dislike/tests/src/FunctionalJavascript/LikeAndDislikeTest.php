...
class LikeAndDislikeTest extends WebDriverTestBase {
  ...
  protected function assertLikesAndDislikes($entity_type_id, $entity_id, $likes = '0', $dislikes = '0', $disabled = FALSE) {
    ...
    $this->assertEquals($this->xpath('//*[@id="' . $like_container_id . '"]/a')[0]->getAttribute('data-entity-type'), $entity_type_id);
    $this->assertEquals($this->xpath('//*[@id="' . $like_container_id . '"]/a')[0]->getAttribute('data-entity-id'), $entity_id);
    $this->assertEquals($this->xpath('//*[@id="' . $like_container_id . '"]/span[@class="count"]')[0]->getText(), $likes);

    // Assert dislikes.
    $dislike_container_id = 'dislike-container-' . $entity_type_id . '-' . $entity_id;
    $this->assertEquals($this->xpath('//*[@id="' . $dislike_container_id . '"]/a')[0]->getAttribute('data-entity-type'), $entity_type_id);
    $this->assertEquals($this->xpath('//*[@id="' . $dislike_container_id . '"]/a')[0]->getAttribute('data-entity-id'), $entity_id);
    $this->assertEquals($this->xpath('//*[@id="' . $dislike_container_id . '"]/span[@class="count"]')[0]->getText(), $dislikes);

    if ($disabled) {
      $this->assertEquals($this->xpath('//*[@id="' . $like_container_id . '"]/a')[0]->getAttribute('class'), 'disable-status');
      $this->assertEquals($this->xpath('//*[@id="' . $dislike_container_id . '"]/a')[0]->getAttribute('class'), 'disable-status');
    }
  }
