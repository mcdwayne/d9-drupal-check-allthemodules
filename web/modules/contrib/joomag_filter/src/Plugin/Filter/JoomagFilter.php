...
/**
 * @file
 * Contains \Drupal\joomag_filter\Plugin\Filter\JoomagFilter.
 */
...
class JoomagFilter {
  ...
  function parser($text) {
    ...
    for ($x = 0; $x < count($matches[0]); $x++) {
      $replace = $this->getMagazine($matches[0][$x]);
    ...
    preg_match_all($regex, $text, $matches);
    for ($x = 0; $x < count($matches[0]); $x++) {
      $replace = $this->getBookshelf($matches[0][$x]);
      $text = str_replace($matches[0][$x], $replace, $text);
    }
   ...
   */
  function getBookshelf($match) {
    $height = $this->get_value_with_default('/(?:^|[\s]+)height=([\S]*)/i', $match, 460);
    $width = $this->get_value_with_default('/(?:^|[\s]+)width=([\S]*)/i', $match, 450);
    $magazine_id = $this->get_value_with_default('/(?:^|[\s]+)magazineId=([\S]*)/i', $match, '');
    $cols = $this->get_value_with_default('/(?:^|[\s]+)cols=([\S]*)/i', $match, 3);
    $rows = $this->get_value_with_default('/(?:^|[\s]+)rows=([\S]*)/i', $match, 2);
   ...
   *   parameters specified in $match.
   */
  function getMagazine($match) {
    $height = $this->get_value_with_default('/(?:^|[\s]+)height=([\S]*)/i', $match, 272);
    $width = $this->get_value_with_default('/(?:^|[\s]+)width=([\S]*)/i', $match, 420);
    $page_number = $this->get_value_with_default('/(?:^|[\s]+)pageNumber=([\S]*)/i', $match, 1);
    $magazine_id = $this->get_value_with_default('/(?:^|[\s]+)magazineId=([\S]*)/i', $match, '');
    $title = $this->get_value_with_default('/(?:^|[\s]+)title=([\S]*)/i', $match, '');
    $background_color = $this->get_value_with_default('/(?:^|[\s]+)backgroundColor=([\S]*)/i', $match, '');
    $background_img = $this->get_value_with_default('/(?:^|[\s]+)backgroundImage=([\S]*)/i', $match, '');
    $toolbar = $this->get_value_with_default('/(?:^|[\s]+)toolbar=([\S]*)/i', $match, '');
    $auto_flip = $this->get_value_with_default('/(?:^|[\s]+)autoFlip=([\S]*)/i', $match, '');
    $auto_fit = $this->get_value_with_default('/(?:^|[\s]+)autoFit=([\S]*)/i', $match, 'FALSE') == 'TRUE' ? TRUE : FALSE;
    ...
    $embed_opts = array();
    $this->get_embed_opts_toolbar($embed_opts, $toolbar);
    $this->get_embed_opts_bkg_color($embed_opts, $background_img, $background_color);
   ...
   */
  private function get_value_with_default($regex, $params, $default) {
    $matches = array();
   ...
   *   or 'transparent'.
   */
  private function get_embed_opts_toolbar(array &$embed_opts, $toolbar) {
    if ($toolbar != '') {
      switch ($toolbar) {
        case 'none':
   ...
   */
  private function get_embed_opts_bkg_color(array &$embed_opts, $background_img, $background_color) {
    if ($background_color != '') {
