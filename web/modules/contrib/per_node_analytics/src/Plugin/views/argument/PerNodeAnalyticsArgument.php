...
namespace Drupal\per_node_analytics\Plugin\views\argument;
 ...
 *
 * @ViewsArgument("per_node_analytics_argument")
 */
class PerNodeAnalyticsArgument extends ArgumentPluginBase {
  ...
  public function query($group_by = FALSE) {
    ...
    if (strpos(\Drupal::routeMatch()
        ->getRouteName(), 'hidden_tab.') === 0 && strpos(\Drupal::routeMatch()
        ->getRouteObject()
        ->getPath(), '/node/{node}/') === 0) {
      $node = \Drupal::request()->get('node');
      if ($node) {
        $this->argument = $node->toUrl()->toString();
      }
    }
