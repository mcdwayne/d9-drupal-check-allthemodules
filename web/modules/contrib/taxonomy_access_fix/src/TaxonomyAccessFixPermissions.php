...
class TaxonomyAccessFixPermissions {
  ...
  public static function fixAccess($op, $vocabulary = NULL) {
    ...
    switch ($op) {
      case 'index':
        // Taxonomy overview page at "/admin/structure/taxonomy".
        // Allow access when the user
        // - has 'access taxonomy overview' permission AND
        // - has access to at least one vocabulary.
        if (\Drupal::currentUser()->hasPermission('access taxonomy overview')) {
      ...
      case 'list terms':
        if ($vocabulary) {
          ...
          $perm3 = sprintf('create terms in %s', $vid);
      ...
      case 'add terms':
        if ($vocabulary) {
          if (\Drupal::currentUser()
            ->hasPermission('create terms in ' . $vocabulary->id())) {
            return TRUE;
          }
        }
        break;
