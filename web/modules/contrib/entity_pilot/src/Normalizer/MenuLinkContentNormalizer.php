      ...
      foreach ($data['link'] as $key => $link) {
        try {
          ...
          if (isset($link['target_uuid'])) {
            $link['target_uuid'];
            $uuid = ['uuid' => $link['target_uuid']];
            if ($entity = $this->unsavedUuid->resolve($this->uuidReference, $uuid, $stub->getEntityTypeId())) {
              $data['link'][$key]['uri'] = 'base:entity_pilot/' . $stub->getEntityTypeId() . '/' . $link['target_uuid'];
            }
