 ...
 * This code is a copy of color.js, with changes on lines 26, 36 and 54.
 */
...
(function ($, Drupal) {
   ...
   */
  Drupal.behaviors.whitelabel = {
    attach: function (context, settings) {
      var i;
      var j;
      var colors;
      // This behavior attaches by ID, so is only valid once on a page.
      var form = $(context).find('.whitelabel-form .color-form').once('whitelabel');
      if (form.length === 0) {
        return;
      }
      var inputs = [];
      ...
      // Add Farbtastic.
      $('<div class="color-placeholder"></div>').once('whitelabel').prependTo(form);
      ...
      for (i in settings.gradients) {
        if (settings.gradients.hasOwnProperty(i)) {
          ...
          $('.color-preview').once('whitelabel').append('<div id="gradient-' + i + '"></div>');
