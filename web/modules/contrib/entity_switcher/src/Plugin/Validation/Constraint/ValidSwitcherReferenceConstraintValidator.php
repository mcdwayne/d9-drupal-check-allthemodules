...
/**
 * Checks if referenced entity switchers are valid.
 */
class ValidSwitcherReferenceConstraintValidator extends ConstraintValidator implements ContainerInjectionInterface {
  ...
  /**
   * Constructs a ValidSwitcherReferenceConstraintValidator object.
   *
  ...
  public function validate($value, Constraint $constraint) {
    ...
    /** @var ValidSwitcherReferenceConstraint $constraint */
    if (!isset($value)) {
      return;
    }
    ...
    $new_entities = [];
    $target_ids = [];
    foreach ($value as $delta => $element) {
      foreach (['data_off', 'data_on', 'switcher'] as $item) {
        $target_id = $element->{$item . '_id'};
        ...
        if (!$element->isEmpty() && $target_id === NULL) {
          if (!$element->{$item}->isNew()) {
            $this->context->buildViolation($constraint->nullMessage)
              ->atPath((string) $delta)
              ->addViolation();
            return;
          }
          $new_entities[$item][$delta] = $element->{$item};
        }

        // '0' or NULL are considered valid empty references.
        if (!empty($target_id)) {
          $target_ids[$item][$delta] = $target_id;
        }
      }
    }
    ...
    foreach (['data_off', 'data_on', 'switcher'] as $item) {
      ...
      $target_type_id = $value->getFieldDefinition()->getSetting('target_type_' . $item);

      // Add violations on deltas with a new entity that is not valid.
      if (!empty($new_entities[$item])) {
        if ($handler instanceof SelectionWithAutocreateInterface) {
          $valid_new_entities = $handler->validateReferenceableNewEntities($new_entities[$item]);
          $invalid_new_entities = array_diff_key($new_entities[$item], $valid_new_entities);
        }
        else {
          // If the selection handler does not support referencing newly created
          // entities, all of them should be invalidated.
          $invalid_new_entities = $new_entities[$item];
        }
      ...
      if ($target_ids[$item]) {
        $valid_target_ids = $handler->validateReferenceableEntities($target_ids[$item]);
        if ($invalid_target_ids = array_diff($target_ids[$item], $valid_target_ids)) {
