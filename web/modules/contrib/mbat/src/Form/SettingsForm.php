...
class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = $this->config('mbat.settings');
    // Renamed from the now meaningless option "determine_menu":
    $form['module_enabled'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Enable the MBAT module'),
      '#description' => $this->t(
      'Use menu the page belongs to, or the page for the taxonomy of which it is a member, for the breadcrumb. If unset, no breadcrumbs are generated or cached and all settings below are ignored.'
      ),
      '#default_value' => $config->get('module_enabled'),
    ];
    ...
    // The following two new options are added beyond Menu Breadcrumb:
    ...
    $form['include_exclude']['mbat_menus'] = [
      '#type' => 'table',
      '#header' => [
        $this->t('Menu'),
        $this->t('Enabled'),
    ...
    foreach ($this->getSortedMenus() as $menu_name => $menu_config) {
      ...
      $form['include_exclude']['mbat_menus'][$menu_name] = [
        '#attributes' => [
          'class' => ['draggable'],
        ],
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $this->config('mbat.settings')
      ->set('module_enabled', (boolean) $form_state->getValue('module_enabled'))
      ...
      ->set('mbat_menus', $form_state->getValue('mbat_menus'))
      ->save();
  ...
  protected function getSortedMenus() {
    ...
    $mbat_menus = $this->config('mbat.settings')->get('mbat_menus');

    foreach ($menus as $menu_name => &$menu) {
      if (!empty($mbat_menus[$menu_name])) {
        $menu = $mbat_menus[$menu_name] + ['label' => $menu];
      }
      else {
        $menu = [
          'weight' => 0,
