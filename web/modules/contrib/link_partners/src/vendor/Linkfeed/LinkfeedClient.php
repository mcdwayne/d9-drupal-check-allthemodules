...
namespace Drupal\link_partners\vendor\Linkfeed;

use Drupal\link_partners\vendor\OpDbAbstract;

class LinkfeedClient extends OpDbAbstract {

  var $lc_version = '0.4.3';
  ...
  function load_links() {
    ...
    $linkfeed_id = \Drupal::config('link_partners.settings')
      ->get('linkfeed.id');
    $host = $this->lc_host;

    if ($this->lc_multi_site) {
      $this->lc_links_db_file = \Drupal::service('file_system')
        ->realpath(file_default_scheme() . "://link_partners/linkfeed/$linkfeed_id/linkfeed.$host.links.db");
    }
    else {
      $this->lc_links_db_file = \Drupal::service('file_system')
        ->realpath(file_default_scheme() . "://link_partners/linkfeed/$linkfeed_id/linkfeed.links.db");
    }
    ...
    if (!$links) {
      ...
      if (!$this->lc_links = @unserialize($links)) {
        $this->lc_links = [];
  ...
  function uptolike_tag() {
    $uptolike_hash = 'lf' . sha1($this->lc_host);
