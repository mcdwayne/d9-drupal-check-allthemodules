(function ($) {
    ...
    function generateNordeaKeys(url, type, id) {
        $.ajax({
            url: url,
            method: 'POST',
            data: {
    ...
    function initModal() {
        // Get the modal
        var modal = document.getElementById("nordea-summary-modal");

        // Get the button that opens the modal
        var btn = document.getElementById("nordea-summary-modal-trigger");
    ...
    $(document).ready(function () {
        ...
        $('button[name^="refresh_nordea_keys_"]').on('click', function (e) {
            e.preventDefault();
            var $this = $(this);
            if (confirm($this.attr('data-confirm_msg'))) {
                generateNordeaKeys($this.attr('data-url'), $this.attr('data-type'), $('#edit-id').val());
            }
        });
