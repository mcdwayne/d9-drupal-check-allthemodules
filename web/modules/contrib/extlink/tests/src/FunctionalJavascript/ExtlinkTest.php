...
class ExtlinkTest extends ExtlinkTestBase {
  ...
  */
  public function testExtlinkImg() {
    ...
    $page = $this->getSession()->getPage();
    $this->assertSession()->statusCodeEquals(200);
    $this->createScreenshot(\Drupal::root() . '/sites/default/files/simpletest/ExtlinkImg.png');
  ...
  public function testExtlinkUseFontAwesome() {
    ...
    $externalLink = $page->find('css', 'span.fa-external-link');
    $this->assertTrue(!is_null($externalLink) && $externalLink->isVisible(), 'Font Awesome span tag is missing.');

    // Test that the page has the Mailto external link span.
    $mailToLink = $page->find('css', 'span.fa-envelope-o');
    $this->assertTrue(!is_null($mailToLink) && $mailToLink->isVisible(), 'Font Awesome span tag is missing.');
  }
