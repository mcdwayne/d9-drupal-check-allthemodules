...
/**
 * @file
 * Contains \Drupal\redirect_deleted_entities\Plugin\RedirectType\EntityRedirectTypeBase.
 */
...
use Drupal\redirect_deleted_entities\RedirectTypeInterface;
use Symfony\Component\DependencyInjection\ContainerInterface;

/**
 * A base class for Redirect Type plugins.
 */
abstract class EntityRedirectTypeBase extends PluginBase implements RedirectTypeInterface {
    ...
    if ($this->entityManager->getDefinition($this->getPluginId())->hasKey('bundle')) {
      foreach ($this->getBundles() as $bundle => $bundle_label) {
        if (count($languages) && $this->isContentTranslationEnabled($bundle)) {
          $patterns[$bundle] = $this->t('Default redirect pattern for @bundle (applies to all @bundle fields with blank patterns below)', array('@bundle' => $bundle_label));
          foreach ($languages as $language) {
            $patterns[$bundle . '_' . $language->getId()] = $this->t('Pattern for all @language @bundle redirects', array(
              '@bundle' => $bundle_label,
          ...
          $patterns[$bundle] = $this->t('Pattern for all @bundle redirects', array('@bundle' => $bundle_label));
        }
      }
    }
    return $patterns;
