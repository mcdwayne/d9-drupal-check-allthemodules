...
/**
 * Test "salt for email domain" -registration code.
 *
 * @group regcode_simple
 */
class RegcodeSimpleCodeSaltEmailDomainTest extends BrowserTestBase {
  ...
  public function testCodePlainText() {
    ...
    $this->getSession()->getPage()->fillField('regcode_type', 'code_salt_email_domain');
    $this->getSession()->getPage()->fillField('code_salt_email_domain', 'emaildomain-salting');
    ...
    $this->getSession()->getPage()->fillField('mail', 'user.without.code@' . $domain);
    ...
    $this->getSession()->getPage()->fillField('mail', 'user.with.invalid.code@' . $domain);
    $this->getSession()->getPage()->fillField('name', 'name.with.invalid.code');
    $this->getSession()->getPage()->fillField('regcode_simple', 'emaildomain-salting');
    $this->getSession()->getPage()->pressButton('Create new account');
    ...
    $code = md5($domain . 'emaildomain-salting');
    $this->getSession()->getPage()->fillField('mail', 'user.with.code@' . $domain);
