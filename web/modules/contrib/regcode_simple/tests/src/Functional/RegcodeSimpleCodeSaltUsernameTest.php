...
/**
 * Test "salt for username" -registration code.
 *
 * @group regcode_simple
 */
class RegcodeSimpleCodeSaltUsernameTest extends BrowserTestBase {
  ...
  public function testCodePlainText() {
    ...
    $perms = ['access administration pages', 'administer account settings'];
    ...
    $this->getSession()->getPage()->fillField('regcode_type', 'code_salt_username');
    $this->getSession()->getPage()->fillField('code_salt_username', 'username-salting');
    ...
    $this->getSession()->getPage()->fillField('regcode_simple', 'username-salting');
    $this->getSession()->getPage()->pressButton('Create new account');
    ...
    $name = 'name.with.code';
    $code = md5($name . 'username-salting');
