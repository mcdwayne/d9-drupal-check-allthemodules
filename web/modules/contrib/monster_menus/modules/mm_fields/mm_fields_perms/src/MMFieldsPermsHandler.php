 ...
 * @ingroup entity_api
 */
class MMFieldsPermsHandler implements MMFieldsPermsHandlerInterface, EntityHandlerInterface {
   ...
   *   The current user.
   */
  public function __construct(EntityTypeInterface $entity_type, EntityManagerInterface $entity_manager, AccountInterface $current_user) {
  ...
  public function entityConfigFormAlter(array &$form, FormStateInterface $form_state, EntityInterface $entity) {
    ...
    if ($new_translation || $has_translations) {
      ...
      $date = $new_translation ? mm_request_time() : $metadata->getCreatedTime();
      $form['content_translation']['created'] = [
        '#type' => 'textfield',
        '#title' => t('Authored on'),
        '#maxlength' => 25,
        '#description' => t('Format: %time. The date format is YYYY-MM-DD and %timezone is the time zone offset from UTC. Leave blank to use the time of form submission.', ['%time' => format_date(mm_request_time(), 'custom', 'Y-m-d H:i:s O'), '%timezone' => format_date(mm_request_time(), 'custom', 'O')]),
  ...
  public function entityFormEntityBuild($entity_type, EntityInterface $entity, array $form, FormStateInterface $form_state) {
    ...
    $metadata = $this->manager->getTranslationMetadata($entity);
    $metadata->setAuthor(!empty($values['uid']) ? User::load($values['uid']) : User::getAnonymousUser();
    $metadata->setPublished(!empty($values['status']));
    $metadata->setCreatedTime(!empty($values['created']) ? strtotime($values['created']) : mm_request_time());
  ...
  public function entityFormSubmit($form, FormStateInterface $form_state) {
    ...
    if ($entity->hasField('content_translation_changed')) {
      $metadata = $this->manager->getTranslationMetadata($entity);
      $metadata->setChangedTime(mm_request_time());
    }
  }
