...
/**
 * Service class for the CloudmersiveNsfw scanner instance.
 *
...
class Scanner {
  ...
  // CloudmersiveNsfw configuration.
   ...
   *
   * @param object $config
   *   An instance of \Drupal\cloudmersivensfw\Config.
   */
  public function __construct(\Drupal\cloudmersivensfw\Config $config) {
    $this->config = $config;
  ...
  /**
   * Check whether the nsfw checks are enabled.
   *
   * @return boolean
  ...
  /**
   * Check whether a specific file should be scanned by CloudmersiveNsfw.
   *
   * Specific files can be excluded from nsfw scanning, such as:
   * - Image files
   ...
   * hook_cloudmersivensfw_file_is_scannable().
   *
   * @see hook_cloudmersivensfw_file_is_scannable().
   *
   * @return boolean
   *    TRUE if a file should be scanned by the nsfw service.
   */
  public function isScannable(FileInterface $file) {
    ...
    // Iterate each module implementing hook_cloudmersivensfw_file_is_scannable().
    // Modules that do not wish to affact the result should return
    // FILE_SCANNABLE_IGNORE.
    foreach (\Drupal::moduleHandler()->getImplementations('cloudmersivensfw_file_is_scannable') as $module) {
      $result = \Drupal::moduleHandler()->invoke($module, 'cloudmersivensfw_file_is_scannable', array($file));
   ...
   *   - CLOUDMERSIVENSFW_SCANRESULT_UNCHECKED
   *     The file was not scanned. The CloudmersiveNsfw service may be unavailable.
   *   - CLOUDMERSIVENSFW_SCANRESULT_CLEAN
   *     The file was scanned, and no infection was found.
   *   - CLOUDMERSIVENSFW_SCANRESULT_INFECTED
  ...
  public function scan(FileInterface $file) {
    ...
    switch ($result) {
      ...
      case self::FILE_IS_INFECTED:
        ...
        \Drupal::logger('Cloudmersive NSFW')->error($message, $replacements);
        break;
      ...
      case self::FILE_IS_UNCHECKED:
        if ($this->config->outage_action() === Config::OUTAGE_ALLOW_UNCHECKED) {
          ...
          \Drupal::logger('Cloudmersive NSFW')->notice($message, $replacements);
        }
        elseif ($verbose_mode) {
  ...
  public static function isSchemeScannable($scheme) {
    ...
    $config = \Drupal::config('cloudmersivensfw.settings');
