...
class DaemonUnixSocket implements ScannerInterface {
  ...
  public function scan(FileInterface $file) {
    // Attempt to open a socket to the CloudmersiveNsfw host and the file.
    $file_handler    = fopen($file->getFileUri(), 'r');
    $scanner_handler = @fsockopen("unix://{$this->_unix_socket}", 0);

    // Abort if the cloudmersivensfw server is unavailable.
    if (!$scanner_handler) {
      \Drupal::logger('Cloudmersive NSFW')->warning('Unable to connect to cloudmersivensfw daemon on unix socket @unix_socket', array('@unix_socket' => $this->_unix_socket));
      return Scanner::FILE_IS_UNCHECKED;
    }

    // Push to the cloudmersivensfw socket.
    $bytes = $file->getSize();
