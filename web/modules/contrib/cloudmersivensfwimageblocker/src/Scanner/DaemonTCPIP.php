...
class DaemonTCPIP implements ScannerInterface {
  ...
  public function scan(FileInterface $file) {
    ...
    // Attempt to open a socket to the CloudmersiveNsfw host.
    $scanner_handler = @fsockopen($this->_hostname, $this->_port);

    // Abort if the CloudmersiveNsfw server is unavailable.
    if (!$scanner_handler) {
      \Drupal::logger('Cloudmersive NSFW')->warning('Unable to connect to Cloudmersive NSFW TCP/IP daemon on @hostname:@port', array('@hostname' => $this->_hostname, '@port' => $this->_port));
