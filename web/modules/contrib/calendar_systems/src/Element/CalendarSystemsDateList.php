...
use Drupal\calendar_systems\CalendarSystems\CalendarSystemsInterface;
...
class CalendarSystemsDateList extends Datelist {
  ...
  public static function valueCallback(&$element, $input, FormStateInterface $form_state) {
    ...
    $calendar = calendar_systems_factory($timezone, 'en');
  ...
  public static function processDatelist(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    $cal = calendar_systems_factory($element['#timezone'], 'en');
    if (!$cal) {
      return parent::processDatelist($element, $form_state, $complete_form);
    }
    $e_cal = calendar_systems_factory($element['#timezone'], 'en', 'gregorian');
    if ($date) {
  ...
  private static function calendarSystemsYears(CalendarSystemsInterface $cal, $min, $max, $required) {
    $cal->setTimestamp(REQUEST_TIME);
    $rng = range(
      empty($min) ? intval($cal->format('Y') - 3) : $min,
      empty($max) ? intval($cal->format('Y') + 3) : $max
  ...
  public static function calendarSystemsDatetimeRangeYears($string, $date, $calendar_name = '') {
    ...
    $calendar = calendar_systems_factory(NULL, 'en', $calendar_name);
    if (!$calendar) {
      return parent::datetimeRangeYears($string, $date);
    }
