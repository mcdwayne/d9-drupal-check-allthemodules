    ...
    foreach ($definitions as $client_id => $client) {
        if (!$this->config('pixelpin_openid_connect.settings.enable')
          ...
          if($customSsoButton['loginButton']){
            $ssoButton = $customSsoButton['loginButton'];
          } else {
            $ssoButton = '<button name="pixelpin" id="edit-openid-connect-client-pixelpin-login-front-page" type="submit" class="ppsso-btn">Log in with <span class="ppsso-logotype">PixelPin</span></button>';
          }
