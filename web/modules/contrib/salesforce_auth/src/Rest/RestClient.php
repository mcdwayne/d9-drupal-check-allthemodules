...
/**
 * Objects, properties, and methods to communicate with the salesforce_auth 
 * REST API.
 */
class RestClient implements RestClientInterface {
    ...
    public function apiCall($path, array $params = [], $method = 'GET', $returnObject = FALSE) {
        ...
        if ($this->response->getStatusCode() == 401) {
            ...
            // refresh token. If refreshToken() throws an exception, 
            // or if apiHttpRequest() throws anything but a RequestException, 
            // let it bubble up.
            $this->refreshToken();
            try {
                $this->response = new RestResponse($this->apiHttpRequest($url, $params, $method));
     ...
     * corresponding RecordType. DeveloperName doesn't change between 
     * salesforce_auth environments, so it's safer to rely on compared to SFID.
     *
     * @param string $name
     *   Object type name, E.g., Contact, Account.
