...
use Drupal\gtm_datalayer\Form\ConditionAddForm;
 ...
 */
class DataLayerFormEditForm extends DataLayerFormAddForm {
  ...
  /**
   * Constructs a DataLayerFormEditForm object.
   *
  ...
  public function addForm(array &$form, FormStateInterface $form_state) {
    ...
    $content['submit']['#attached']['drupalSettings']['ajax'][$content['submit']['#id']]['url'] = Url::fromRoute('entity.gtm_datalayer_form.condition.add', $route_parameters, $options)->toString();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    drupal_set_message($this->t('The GTM dataLayer Form configuration has been updated.'));
    $form_state->setRedirect('entity.gtm_datalayer_form.collection');
  }
  ...
  protected function renderRows() {
    ...
    foreach ($this->entity->getAccessConditions() as $row => $condition) {
      $build = [
        '#type' => 'operations',
        '#links' => $this->getOperations('entity.gtm_datalayer_form.condition', [
          'entity' => $this->entity->id(),
          'id' => $row,
        ]),
      ];
