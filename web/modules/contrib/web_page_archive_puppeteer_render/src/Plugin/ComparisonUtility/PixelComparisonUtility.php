...
use Drupal\web_page_archive_puppeteer_render\Plugin\CompareResponse\PixelScreenshotVarianceCompareResponse;
...
class PixelComparisonUtility extends FilterableComparisonUtilityBase {
  ...
  public function compare(CaptureResponseInterface $a, CaptureResponseInterface $b, array $data = []) {
    ...
    $settings = $this->configFactory->get('web_page_archive.web_page_archive_puppeteer_render_capture.settings');
    $file1 = $this->fileSystem->realpath($a->getContent());
    $file2 = $this->fileSystem->realpath($b->getContent());
    $file_path = $this->getFileName($data, $settings->get('system.image_type'));
  ...
  public function isAvailable() {
    ...
    if (empty($magick) || !file_exists($magick)) {
      \Drupal::logger('web_page_archive_puppeteer_render')->notice(t("Imagick binary is missing"));
      return FALSE;
    }
