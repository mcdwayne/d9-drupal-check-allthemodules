 ...
 * add and edit forms as it does not inherit from AutobanFormBase. The reason
 * for this is that we do not need to build the same form. Instead, we present
 * the user with a simple yes/no question. For this reason, we derive from
...
class AutobanDeleteForm extends EntityConfirmFormBase {
  ...
  public function getConfirmText() {
    return $this->t('Delete Autoban rule');
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    drupal_set_message($this->t('Autoban rule #%label was deleted.', [
      '%label' => $this->entity->id(),
    ]));
