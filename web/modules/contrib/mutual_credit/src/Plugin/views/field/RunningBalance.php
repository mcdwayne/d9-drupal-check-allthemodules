...
class RunningBalance extends Worth {
  ...
  public function render(ResultRow $values) {
    ...
    $vals = $this->getBalance($transaction, $values->{$this->fAlias}, $values->{$this->aliases['running_created']});
   ...
   *   The
   */
  private function getBalance($transaction, $fAlias, $before) {
    $vals = [];
    foreach ($transaction->worth->currencies() as $curr_id) {
      ...
      $vals[] = [
        'curr_id' => $curr_id,
        'value' => $this->txStorage->runningBalance($fAlias, $curr_id, 'created')
      ];
    }
    return $vals;
  }
