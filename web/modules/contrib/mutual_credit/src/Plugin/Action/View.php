...
class View extends TransactionActionBase {
   ...
   */
  public function __construct($configuration, $plugin_id, $plugin_definition, $entity_form_builder, $module_handler, $relative_active_plugins, $entity_type_manager, $entity_display_respository, $current_user, $messenger, RouteMatchInterface $route_match) {
    parent::__construct($configuration, $plugin_id, $plugin_definition, $entity_form_builder, $module_handler, $relative_active_plugins, $entity_type_manager, $entity_display_respository, $current_user, $messenger);
  ...
  public function access($object, AccountInterface $account = NULL, $return_as_object = FALSE) {
    ...
    if ($result->isAllowed()) {
      ...
      $result = $result->forbiddenIf(
        isset($params['mcapi_transaction']) && $params['mcapi_transaction'] == $object->serial->value,
        'You are already viewing this transaction'
      );
    }
