...
class Levy extends RulesActionBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    \Drupal::messenger()->addStatus('At time of writing (d8 alpha 14) there is no way to trigger actions so this action is untested');
    \Drupal::messenger()->addStatus('This tool is a bit crude but it will have to do until someone can figure out how to do it with rules');
    \Drupal::messenger()->addStatus('This action ONLY goes on one the hook, and adds a dependent transaction to the cluster. with the same serial number');
    \Drupal::messenger()->addStatus('It will be easy to make this action more sophisticated');
    $w = 5;
    $form = array(
      'participants' => array(
