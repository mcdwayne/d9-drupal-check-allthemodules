...
class CurrencyForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $admins = \Drupal::entityTypeManager()->getStorage('user')->loadByProperties(['roles' => array_keys(user_roles(TRUE, 'manage mcapi'))]);
    if (count($admins)) {
      $options = [1 => User::load(1)->getUsername()];
      foreach ($admins as $uid => $user) {
        $options[$uid] = $user->getusername();
      }
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status == SAVED_UPDATED) {
      $this->messenger()->addStatus(t('Currency %label has been updated.', ['%label' => $currency->label()]));
    }
    else {
      $this->messenger()->addStatus(t('Currency %label has been added.', ['%label' => $currency->label()]));
    }
