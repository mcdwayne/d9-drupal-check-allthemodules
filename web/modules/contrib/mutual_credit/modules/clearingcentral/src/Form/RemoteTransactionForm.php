...
class RemoteTransactionForm extends TransactionForm {
  ...
  public function __construct($entity_manager, $tempstore, $current_request, $current_user, $logger_channel) {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($response_code == CEN_SUCCESS) {
      ...
      $this->logger->debug('<pre>@array</pre>', ['@array' => print_r($transaction->toArray(), 1)]);
      $transaction->save();
      ...
      $this->messenger()->addError($cc->response($response_code));
      $form_state->setRebuild(TRUE);
    }
  }
