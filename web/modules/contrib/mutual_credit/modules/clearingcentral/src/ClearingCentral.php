...
class ClearingCentral implements IntertradingInterface {
   ...
   * @param AccountSwitcher $account_switcher
   * @param MessengerInterface $messenger
   */
  function __construct(ConfigFactory $config_factory, GuzzleHttp $http_client, KeyValueFactory $key_value, Query $entity_query, LoggerChannel $logger_channel, AccountSwitcher $account_switcher, MessengerInterface $messenger) {
  ...
  public function send(TransactionInterface $transaction, $remote_exchange_id, $remote_user_id) {
    ...
    $this->logger->debug('<pre>@array</pre>', ['@array' =>  print_r($remote_transaction, 1)]);
    try {
    ...
    if ($result->getStatusCode() == 200) {
      ...
      $this->logger->error('@string', ['@string' => $message]);
  ...
  public function receive(&$params) {
    ...
    if (count($violations)) {
      foreach ($violations as $violation) {
        ...
        $this->logger->error('@string', ['@string' => $violation->getMessage()]);
        $bad = TRUE;
      }
    }
    if (isset($bad)) {
