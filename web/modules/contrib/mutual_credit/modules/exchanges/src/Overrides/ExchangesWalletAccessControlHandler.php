 ...
 * 
 * @todo lots more work, and dependency injections
 */
class ExchangesWalletAccessControlHandler extends WalletAccessControlHandler {
  ...
  public function createAccess($entity_bundle = NULL, AccountInterface $account = NULL, array $context = [], $return_as_object = FALSE) {
    ...
    if (\Drupal::currentUser()->id() != $account->id()) {
      \Drupal::messenger()->addStatus("Check that the current user has an exchange in common with ".$account->getDisplayName());
    }
    return $access;
  }
  ...
  public function checkAccess(EntityInterface $entity, $op, AccountInterface $account) {
    ...
    if ($result->isNeutral()) {
      ...
      \Drupal::messenger()->addStatus('@todo rewrite ExchangesWalletAccessControlHandler');
