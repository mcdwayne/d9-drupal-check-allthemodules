...
class ExchangeTransactionDevelGenerate extends TransactionDevelGenerate {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    \Drupal::messenger()->addStatus("This only works once users have been added to exchanges.");
  ...
  public function get2RandWalletIds(array $conditions = []) {
    ...
    shuffle($wids);
    if (count($wids) < 2) {
      \Drupal::logger('mcapi')->warning('Only @num wallets in exchange @ex_id: @wids', ['@num' => count($wids), '@ex_id' => $exchange->id(), '@wids' => print_r($wids, 1)]);
      return [];
    }
