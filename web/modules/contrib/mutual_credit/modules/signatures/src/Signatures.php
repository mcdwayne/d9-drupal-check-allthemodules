...
class Signatures {
  ...
  public static function load($entities) {
    $xids = [];
    // Entities are keyed by xid but singatures are indexed by serial. $xids is
    // a quick lookup table for transactions which are not children.
    ...
    // Load all the signatures for the given serial numbers and append them to
  ...
  public function waitingOn($uid = NULL) {
    ...
    if ($this->transaction->state->target_id == 'pending') {
      if (empty($this->transaction->signatures)) {
        trigger_error('Invalid pending transaction '.$this->transaction->serial->value.' has no signatures.', E_USER_ERROR);
      }
