...
class McapiFormPermission extends ProcessPluginBase implements ContainerFactoryPluginInterface {
   ...
   */
  public function __construct($configuration, $plugin_id, $plugin_definition, PermissionHandler $permissions, PhpTransliteration $transliteration, MessengerInterface $messenger) {
      ...
      case 'user_chooser_segment_roles':
        ...
        if ($d8_role_name = $this->machineName($value->roles[$arg])) {
          if ($role = Role::load($d8_role_name)) {
            ...
            $this->messenger->addWarning("Access to transaction form used to be role '$d8_role_name' but has now been set to permission '$perm'");
            break;
          }
          else {
            $this->messenger->addError("Could not find  expected fole: $d8_role_name");
          }
        }
        else {
            $this->messenger->addError("Could not identify d7 role with ID of ".$arg);
        }
        break;

      default:
        $this->messenger->addWarning("Could not determine a permission for designed transaction form");
    }
