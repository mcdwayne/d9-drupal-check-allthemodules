(function ($, Drupal, drupalSettings) {
  Drupal.behaviors.opignoMoxtraMeetingForm = {
    attach: function (context, settings) {
      const $meeting_members = $('#meeting_members', context);
      const $meeting_members_available = $('#meeting_members-available', context);
      const $meeting_members_autocomplete = $('#meeting_members_autocomplete', context);
      $meeting_members_autocomplete
          .once('autocompleteselect')
          .on('autocompleteselect', function (e, ui) {
            // Get ids of the already selected options.
            const selected_ids = $('option', $meeting_members)
                .map(function () {
                  return $(this).val();
                }).get();

            // Replace available options list with the selected option.
            $meeting_members_available.empty();
            if (selected_ids.indexOf(ui.item.id) === -1) {
              const option_html = '<option value="' + ui.item.id + '">'
                  + ui.item.label
                  + '</option>';
              $meeting_members_available.append(option_html);
            }
          });
      $meeting_members_autocomplete
