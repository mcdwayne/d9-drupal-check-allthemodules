...
namespace Drupal\micro_sitemap\Access;
...
class MicroSitemapAccess {
  ...
  public function access(Route $route, RouteMatchInterface $route_match, AccountInterface $account, SiteInterface $site = NULL) {
    ...
    if (!$site->isRegistered()) {
      return AccessResult::forbidden('Vocabulary can be managed only on site registered and so from the site url.')->addCacheableDependency($site);
    }

    if ($account->hasPermission('administer micro sites sitemap')) {
      return AccessResult::allowed()->cachePerPermissions();
    }

    if ($account->hasPermission('administer own micro site sitemap')) {
