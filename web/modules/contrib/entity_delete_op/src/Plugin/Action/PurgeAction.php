...
/**
 * Defines the "Purge" action.
 *
 * @Action(
 *   id = "entity:entity_delete_op_purge",
 *   action_label = @Translation("Entity delete op: purge"),
 *   deriver = "Drupal\entity_delete_op\Plugin\Action\Derivative\DeletableEntityActionDeriver"
 * )
 */
class PurgeAction extends EntityActionBase {
  ...
  public function execute($entity = NULL) {
    if ($entity instanceof EntityDeletableInterface) {
      $this->deleteManager->purge($entity);
    }
  }
  ...
  public function access($object, AccountInterface $account = NULL, $return_as_object = FALSE) {
    ...
    $permissions = [
      'entity_delete_op purge any ' . $object->getEntityTypeId() . ' entities',
    ];

    if ($object instanceof EntityOwnerInterface) {
      if ($account->id() == $object->getOwnerId()) {
        $permissions[] = 'entity_delete_op purge own ' . $object->getEntityTypeId() . ' entities';
      }
    }
