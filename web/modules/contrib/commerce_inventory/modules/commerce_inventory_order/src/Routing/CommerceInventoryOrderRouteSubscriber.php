...
namespace Drupal\commerce_inventory_order\Routing;
...
/**
 * Route changes for the commerce inventory order module.
 */
class CommerceInventoryOrderRouteSubscriber extends RouteSubscriberBase {
  ...
  protected function alterRoutes(RouteCollection $collection) {
    ...
    if ($inventory_location_adjustments_route = $this->getInventoryLocationAdjustmentsRoute($entity_type_location, $collection)) {
      $collection->add("entity.{$entity_type_location_id}.inventory_adjustments_order_holds", $inventory_location_adjustments_route);
    }
    ...
    if ($inventory_item_adjustments_route = $this->getInventoryItemAdjustmentsRoute($entity_type_item, $collection)) {
      $collection->add("entity.{$entity_type_item_id}.adjustments_order_holds", $inventory_item_adjustments_route);
    }
  ...
  protected function getInventoryLocationAdjustmentsRoute(EntityTypeInterface $entity_type, RouteCollection $collection) {
    $view_route_name = 'view.commerce_order_item_inventory_adjustment_holds.admin_commerce_inventory_location';
    $entity_route_name = 'inventory-adjustments-order-holds';
  ...
  protected function getInventoryItemAdjustmentsRoute(EntityTypeInterface $entity_type, RouteCollection $collection) {
    $view_route_name = 'view.commerce_order_item_inventory_adjustment_holds.admin_commerce_inventory_item';
    $entity_route_name = 'adjustments-order-holds';
