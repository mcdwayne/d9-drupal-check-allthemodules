...
namespace Drupal\ep\Plugin\migrate\id_map;
...
/**
 * Defines the Elastic Path sync migration map.
 *
 * It creates one map and one message table per Elastic path sync to store the
...
class Sync extends Sql {
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, MigrationInterface $migration, EventDispatcherInterface $event_dispatcher) {
    ...
    // Remove ep_ from the migration ID so it is not duplicated.
    $machine_name = str_replace([':', 'ep_'], ['__', ''], $this->migration->id());
    $prefix_length = strlen($this->database->tablePrefix());
    $this->mapTableName = 'ep_map_' . mb_strtolower($machine_name);
    $this->mapTableName = mb_substr($this->mapTableName, 0, 63 - $prefix_length);
    $this->messageTableName = 'ep_message_' . mb_strtolower($machine_name);
