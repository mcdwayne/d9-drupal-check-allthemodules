...
class WidgetTest extends KernelTestBase {
  ...
  public function testFieldWidget() {
    foreach ($this->getRangeFieldTypes() as $this->fieldType) {
      ...
      $this->assertText($widget_settings['label']['from']);
      $this->assertText($widget_settings['label']['to']);
      $this->assertText($field_settings['from']['prefix']);
      $this->assertText($field_settings['from']['suffix']);
      $this->assertText($field_settings['to']['prefix']);
      $this->assertText($field_settings['to']['suffix']);
      // Delete field.
      $this->deleteField();
    }
  }
