...
class SnippetListBuilderTest extends UnitTestCase {
  ...
  public function testBuilderHeader() {
    ...
    $expected_header = [
      ...
      'page' => 'Page',
      'block' => 'Block',
      'display_variant' => 'Display variant',
      'layout' => 'Layout',
  ...
  public function testBuilderRow(SnippetInterface $snippet) {
    ...
    $this->assertEquals($row['status']['data'], $snippet->status() ? $this->trans('Enabled') : $this->trans('Disabled'));

    $is_inactive = isset($row['status']['class'][0]) && $row['status']['class'][0] == 'sm-inactive';
    $this->assertEquals($snippet->status(), !$is_inactive);
    $this->assertEquals($row['page']['data'], $snippet->get('page')['status'] ? $snippet->get('page')['path'] : '');
    $this->assertEquals($row['block']['data'], $snippet->get('block')['status'] ? $snippet->get('block')['name'] : '');
  }
  ...
  public function getData() {
    ...
    for ($i = 0; $i < 10; $i++) {
      ...
      $snippet->method('status')->willReturn((bool) mt_rand(0, 1));
      $data[][] = $snippet;
    }
