...
/**
 * Returns responses for 'group_flashpoint_community_content' GroupContent routes.
 */
class GroupFlashpointCommunityContentController extends GroupContentController {
  ...
  /**
   * Constructs a new GroupFlashpointCommunityContentController.
   *
  ...
  public function addPage(GroupInterface $group, $create_mode = FALSE) {
    ...
    $storage_handler = $this->entityTypeManager->getStorage('flashpoint_community_c_type');
    foreach ($this->addPageBundles($group, $create_mode) as $plugin_id => $bundle_name) {
      if (!empty($build['#bundles'][$bundle_name])) {
        ...
        $t_args = ['%flashpoint_community_c_type' => $bundle_label];
        $description = $create_mode
          ? $this->t('Create Flashpoint Community Content of type %flashpoint_community_c_type in the group.', $t_args)
          : $this->t('Add existing Flashpoint Community Content of type %flashpoint_community_c_type to the group.', $t_args);
  ...
  protected function addPageBundles(GroupInterface $group, $create_mode) {
    ...
    // Retrieve all group_flashpoint_community_content plugins for the group's type.
    $plugin_ids = $this->pluginManager->getInstalledIds($group->getGroupType());
    foreach ($plugin_ids as $key => $plugin_id) {
      if (strpos($plugin_id, 'group_flashpoint_community_content:') !== 0) {
