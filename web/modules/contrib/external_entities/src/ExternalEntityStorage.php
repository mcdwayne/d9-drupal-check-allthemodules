...
class ExternalEntityStorage extends ContentEntityStorageBase {
  ...
  protected function getStorageClient($bundle_id) {
    if (!isset($this->storageClients[$bundle_id])) {
      ...
      $this->storageClients[$bundle_id] = $this->storageClientManager
        ->createInstance(
          $bundle->getClient(),
          $config
        );
  ...
  protected function doLoadMultiple(array $ids = NULL) {
    ...
    foreach ($ids as $id) {
      if (strpos($id, '-')) {
        ...
        if ($external_id) {
          $entities[$id] = $this
            ->create([$this->entityType->getKey('bundle') => $bundle])
            ->mapObject($this->getStorageClient($bundle)->load($external_id))
            ->enforceIsNew(FALSE);
        }
      }
    }
